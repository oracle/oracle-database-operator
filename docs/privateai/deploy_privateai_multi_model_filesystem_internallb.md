...
3. Use the file [pai_secret_update_files.sh](./provisioning/pai_secret_update_files.sh) to:

- Generate a new set of required keys and an SSL certificate but while using this script, use the IP noted in Step 2 for `common name` while generating the SSL certificate.
- Encode these files using `base64` and write them to a file named `secretupdate.yaml` in the below format:
...
4. Use the below command to patch the secret `paisecret`. It will replace the Internal LoadBalancer Certificate:
    ```sh
...
**NOTE:** The file `/privateai/config/config.json` inside the running Kubernetes Pod will have the details of the AI Models currently Deployed. You can use the below steps to confirm:
```sh
