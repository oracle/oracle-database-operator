...
- The old build-side `05-*service.yaml` examples are intentionally not copied here because SIDB now renders the `*-lb` LoadBalancer Service through `spec.services.endpoints`.
...
- each `*-lb` Service exists and has an internal load balancer IP
