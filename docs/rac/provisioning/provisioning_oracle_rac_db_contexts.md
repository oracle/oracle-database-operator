# Provisioning an Oracle RAC Database
...
* The .yaml file also includes security contexts to control Kernel Level Parameters at the Kubernetes Pods (RAC Nodes) level.
* This example uses `racdb_prov_contxts.yaml` to provision an Oracle RAC Database using Oracle RAC Controller. The provisioning includes:
...
### In this example, 
  ...
  * If you plan to build the image yourself, you can build using the files from this [GitHub location](https://github.com/oracle/docker-images/tree/main/OracleDatabase/RAC/OracleRealApplicationClusters#building-oracle-rac-database-container-slim-image). In this case, you will need to change value of `image` with the image you have built in your enviornment in file `racdb_prov_contxts.yaml`. 
...
Use the file: [racdb_prov_contxts.yaml](./racdb_prov_contxts.yaml) for this use case as below:

1. Deploy the `racdb_prov_contxts.yaml` file:
    ```sh
    kubectl apply -f racdb_prov_contxts.yaml
    ```
2. Check the status of the deployment:
    ...
    ===================================
    ```
3. Samples logs in [Logs](./logs/racdb_prov_contxts/racdbprov-sample_details.txt) and the corresponding [DB Operator Logs](./logs/racdb_prov_contxts/operator_logs.txt) when the above YAML file is applied.