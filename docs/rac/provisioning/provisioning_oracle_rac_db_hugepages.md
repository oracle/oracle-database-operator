...
* This example uses `racdb_prov_hugepages.yaml` to provision an Oracle RAC Database using Oracle RAC Controller. The provisioning includes:
...
### In this example, 
  ...
  * If you plan to build the image yourself, you can build using the files from this [GitHub location](https://github.com/oracle/docker-images/tree/main/OracleDatabase/RAC/OracleRealApplicationClusters#building-oracle-rac-database-container-slim-image). In this case, you will need to change value of `image` with the image you have built in your enviornment in file `racdb_prov_hugepages.yaml`. 
...
Use the file: [racdb_prov_hugepages.yaml](./racdb_prov_hugepages.yaml) for this use case as below:
...
2. Check the status of the deployment:
    ...
    ===================================
    ```
3. Samples logs in [Logs](./logs/racdb_prov_hugepages/racdbprov-sample_details.txt) and the corresponding [DB Operator Logs](./logs/racdb_prov_hugepages/operator_logs.txt) when the above YAML file is applied. 