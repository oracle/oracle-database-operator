# Provisioning an Oracle RAC Database with RU Patch
...
* In this use case, before the installation, the GRID HOME and RDBMS HOME are patched with the provided RU Patch. 
* This example uses `racdb_prov_rupatch.yaml` to provision an Oracle RAC Database using Oracle RAC Controller. The provisioning includes:  
  ...
  * Directory where the Release Update (RU) patch has been unzipped is specified by `ruPatchLocation`.
    + For Example: To apply the 19.29 RU Patch `38298204`, if you have unzipped the RU Patch .zip file `p38298204_190000_Linux-x86-64.zip` to location `/scratch/software/stage/19c/19.29/RU` on the worker node, then set this parameter `ruPatchLocation` to value `/scratch/software/stage/19c/19.29/RU/38298204`.
...
### In this example, 
  ...
  * If you plan to build the image yourself, you can build using the files from this [GitHub location](https://github.com/oracle/docker-images/tree/main/OracleDatabase/RAC/OracleRealApplicationClusters#building-oracle-rac-database-container-slim-image). In this case, you will need to change value of `image` with the image you have built in your enviornment in file `racdb_prov_rupatch.yaml`. 
...
### Steps: Deploy 2 Node Oracle RAC Database with RU Patch 
Use the file: [racdb_prov_rupatch.yaml](./racdb_prov_rupatch.yaml) for this use case as below:

1. Deploy the `racdb_prov_rupatch.yaml` file:
    ```sh
    kubectl apply -f racdb_prov_rupatch.yaml
    ```
2. Check the status of the deployment:
    ...
    ===================================
    ```
3. Samples logs in [Logs](./logs/racdb_prov_rupatch/racdbprov-sample_details.txt) and the corresponding [DB Operator Logs](./logs/racdb_prov_rupatch/operator_logs.txt) when the above YAML file is applied. 