...
* This example uses [racdb_prov_pvc.yaml](./racdb_prov_pvc.yaml) to provision an Oracle RAC Database with PVC-based software locations and OpenSSL-encrypted secrets.
* The sample includes:
  ...
  * OpenSSL encrypted database credentials referenced through `dbSecret.keyFileName`, `dbSecret.pwdFileName`, `dbSecret.encryptionType`, and `dbSecret.pkeyopt`
  * Namespace: `rac`

### In This Example
* The sample uses the image `dbocir/oracle/database-rac:19.3.0-slim`.
