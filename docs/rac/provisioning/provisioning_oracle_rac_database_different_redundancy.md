...
* The diskgroups are having different redundancy levels. 
...
### Steps: Deploy 2 Node Oracle RAC Database with different diskgroups with different redundancy levels
Use the file: [racdb_prov_diff_dg_for_db_and_crs_with_diff_redundancy.yaml](./racdb_prov_diff_dg_for_db_and_crs_with_diff_redundancy.yaml) for this use case as below:
...
3. Once the deployment is completed, check the details of the diskgroups:
    ...
    ```
4. Samples logs in [Logs](./logs/racdb_prov_diff_dg_diff_redundancy/racdbprov-sample_details.txt) and the corresponding [DB Operator Logs](./logs/racdb_prov_diff_dg_diff_redundancy/operator_logs.txt) when the above YAML file is applied. 
