...
* [Additional Labels, Annotations, and Resources](./examples/metadata_resources.md)
* [Automatic Pool Probing (Experimental)](./examples/probing.md)
...
* **Automatic Pool Probing (Experimental)**
Added automatic pool probing for configured pools, with probe results and aggregated pool health exposed in the `OrdsSrvs` resource status. Configure the probing interval with `spec.poolProbeIntervalSeconds`.  
[Probing Example](./examples/probing.md)  
  **⚠️Warning⚠️:** Automatic pool probing is currently experimental. It is disabled by default, not available with Central Configuration Server, and the current implementation uses HTTPS probing without certificate verification.  
