# Deploy Oracle GDD with Composite Sharding and Data Guard replication using a minimal configuration
...
This example uses `composite_shard_prov.yaml` manifest to provision an Oracle GDD deployment with the Oracle Sharding Controller using:
...
[`composite_shard_prov.yaml`](./composite_shard_prov.yaml)

1. Deploy the `composite_shard_prov.yaml` manifest:

    ```sh
    kubectl apply -f composite_shard_prov.yaml
    ```
...
4. After the databases are ready, monitor the sharding setup log for the catalog database pod:
    ...
    Monitor the sharding setup log for each shard database pod. For example, for `cpsp1-0`:

    ```sh
    kubectl exec pod/cpsp1-0 -n shns -- \
      /bin/bash -c "tail -f /var/tmp/gdd/oracle_sharding_setup.log"
    ```
