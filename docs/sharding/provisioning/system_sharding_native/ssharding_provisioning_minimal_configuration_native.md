# Deploy Oracle GDD with System-Managed Sharding and Native (Raft) replication using a minimal configuration
...
This example uses `ssharding_shard_prov_native.yaml` to provision an Oracle GDD system with the Oracle Sharding Controller using:
...
Use this manifest: [`ssharding_shard_prov_native.yaml`](./ssharding_shard_prov_native.yaml)

1. Deploy the `ssharding_shard_prov_native.yaml` manifest:

    ```sh
    kubectl apply -f ssharding_shard_prov_native.yaml
    ```
