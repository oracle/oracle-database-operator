# Scale in an existing Oracle GDD deployment with System-Managed Sharding and Native (Raft) replication
...
This use case demonstrates how to delete a shard from an existing Oracle GDD deployment with System-Managed Sharding and Native (Raft) replication that was provisioned using the Oracle Sharding Controller.
...
* Five shard database pods (`shardNum: 5`)
...
Scale in the deployment by changing `shardNum` from `5` to `4` in the manifest and applying the updated configuration.

Use the following manifest for this deployment:

[ssharding_shard_prov_delshard_native.yaml](./ssharding_shard_prov_delshard_native.yaml)

1. Deploy the `ssharding_shard_prov_delshard_native.yaml` manifest:

    ```sh
    kubectl apply -f ssharding_shard_prov_delshard_native.yaml
    ```
...
2. Check the status of the deployment:
    ...
    **NOTE:** After you apply `ssharding_shard_prov_delshard_native.yaml`, the change may not be visible immediately. The shard is removed only after all its chunks have been relocated and required Replication Unit (RU) role changes are completed.

    To monitor the chunk movement and status of RUs, use the following command:
