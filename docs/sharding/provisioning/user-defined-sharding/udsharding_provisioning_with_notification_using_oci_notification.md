...
4. Create a key file `privatekey` that has the PEM key of the OCI user being used to send notification:
    ```sh
    -----BEGIN PRIVATE KEY-G----
...
In this example, we are using pre-built Oracle Database and Global Data Services container images available on the [Oracle Container Registry](https://container-registry.oracle.com/)
  ...
  * If you plan to build and use the images, then you must exchanage the `dbImage` and `gsmImage` tags for the images that you have built in your enviornment in file `udsharding_shard_prov_send_notification.yaml`. 
