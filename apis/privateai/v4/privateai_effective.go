...
func effectiveService(spec *PrivateAiSpec) *PaiServiceSpec {
	if spec.Networking != nil && spec.Networking.Service != nil {
		service := spec.Networking.Service.DeepCopy()
		normalizeServicePorts(service)
		return service
	}
	...
	if len(spec.PortMappings) > 0 {
		...
		if len(service.Ports) == 0 {
			service.Ports = servicePortsFromMappings(spec.PortMappings)
		}
	}
...
func isPaiServiceSpecZero(in PaiServiceSpec) bool {
	return len(in.Ports) == 0 &&
		in.Cluster == nil &&
		in.PublicLoadBalancer == nil &&
		in.PrivateLoadBalancer == nil &&
		len(in.PortMappings) == 0 &&
		in.SvcName == "" &&
		in.SvcType == ""
}

func normalizeServicePorts(service *PaiServiceSpec) {
	if service == nil || len(service.Ports) > 0 || len(service.PortMappings) == 0 {
		return
	}
	service.Ports = servicePortsFromMappings(service.PortMappings)
}

func servicePortsFromMappings(mappings []PaiPortMapping) []PaiServicePortSpec {
	ports := make([]PaiServicePortSpec, 0, len(mappings))
	for _, mapping := range mappings {
		ports = append(ports, PaiServicePortSpec{
			Port:       mapping.Port,
