...
func TestEffectivePrivateAiSpecFromSpec_FallsBackToLegacyPortMappings(t *testing.T) {
	spec := &PrivateAiSpec{
		PaiService: PaiServiceSpec{
			PublicLoadBalancer: &PaiLoadBalancerSpec{
				IP: "1.2.3.4",
			},
		},
	...
	if len(effective.Networking.Service.Ports) != 1 {
		t.Fatalf("expected legacy top-level portMappings to be converted to service ports, got %#v", effective.Networking.Service.Ports)
	}
	if effective.Networking.Service.Ports[0].Port != 443 || effective.Networking.Service.Ports[0].TargetPort != 8443 {
		t.Fatalf("unexpected effective service port: %#v", effective.Networking.Service.Ports[0])
	}
	if effective.Networking.Service.PublicLoadBalancer == nil || effective.Networking.Service.PublicLoadBalancer.IP != "1.2.3.4" {
		t.Fatalf("expected publicLoadBalancer to be preserved, got %#v", effective.Networking.Service.PublicLoadBalancer)
	}
}
