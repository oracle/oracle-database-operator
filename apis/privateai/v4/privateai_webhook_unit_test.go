...
func TestValidateTrafficManagerRoutePathRejectsUnsafeNginxSyntax(t *testing.T) {
	badPaths := []string{
		"/pai a/v1/",
		"/pai/v1/;return 200;/",
		"/pai/v1/{/",
		"/pai/v1/\n/still-ends-with-slash/",
	}

	for _, path := range badPaths {
		if err := ValidateTrafficManagerRoutePath(path); err == nil {
			t.Fatalf("expected route path %q to be rejected", path)
		}
	}
}

func TestValidateTrafficManagerRoutePathAllowsDefaultShape(t *testing.T) {
	goodPaths := []string{
		"",
		"/pai-sample/v1/",
		"/pai_sample/v1.0/",
	...
	for _, path := range goodPaths {
		if err := ValidateTrafficManagerRoutePath(path); err != nil {
			t.Fatalf("expected route path %q to be allowed, got %v", path, err)
		}
	}
}
...
func TestPrivateAIDefault_DefaultsHierarchicalSecretAndTLSMountLocationToSharedSSLPath(t *testing.T) {
	validator := &PrivateAi{}
	obj := &PrivateAi{
		Spec: PrivateAiSpec{
			Security: &PrivateAiSecuritySpec{
	...
	if err := validator.Default(context.Background(), obj); err != nil {
		t.Fatalf("expected defaulting to succeed, got error: %v", err)
	}
	if got := obj.Spec.Security.Secret.MountLocation; got != "/privateai/ssl" {
		t.Fatalf("expected auth secret mountLocation /privateai/ssl, got %q", got)
	}
	if got := obj.Spec.Security.TLS.MountLocation; got != "/privateai/ssl" {
		t.Fatalf("expected TLS mountLocation /privateai/ssl, got %q", got)
	}
}
...
func TestDeprecatedPrivateAIFieldWarnings_UseGroupedFieldNames(t *testing.T) {
	...
	expected := []string{
		"spec.networking.service.publicLoadBalancer.enabled",
		"spec.networking.service.ports",
		"spec.networking.service.publicLoadBalancer.annotations",
	}
