...
type ShardSpec struct {
	...
	StorageSource *StorageSource    `json:"storageSource,omitempty"` // Optional source for the primary shard volume
	// Deprecated: no longer used by the operator. Use additionalPVCs instead.
...
type ShardingDetails struct {
	...
	PrimaryDatabaseRef         *DatabaseRef                 `json:"primaryDatabaseRef,omitempty"`
	StandbyConfig              *StandbyConfig               `json:"standbyConfig,omitempty"`
	ExternalPrimarySources     []StandbyPrimarySource       `json:"externalPrimarySources,omitempty"`
	EnvVars                    []EnvironmentVariable        `json:"envVars,omitempty"`
	Resources                  *corev1.ResourceRequirements `json:"resources,omitempty" protobuf:"bytes,1,opt,name=resources"`
	Nodes                      []string                     `json:"nodes,omitempty"`
...
// StorageSource identifies an optional provider-backed source for the primary
// database volume. It is intentionally sharding-specific and is not shared by
// SingleInstanceDatabase persistence types.
type StorageSource struct {
	OCI *OCIStorageSource `json:"oci,omitempty"`
}

// OCIStorageSource identifies an OCI Block Volume Backup used to provision a
// new primary database volume.
type OCIStorageSource struct {
