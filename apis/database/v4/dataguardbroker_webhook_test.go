...
func TestDataguardBrokerWebhookValidateCreateRejectsCrossNamespaceLocalRef(t *testing.T) {
	obj := validDataguardBrokerTopology()
	obj.Namespace = "broker-ns"
	obj.Spec.Topology.Members[0].LocalRef.Namespace = "other-ns"

	_, err := (&DataguardBroker{}).ValidateCreate(context.Background(), obj)
	if err == nil {
		t.Fatalf("expected cross-namespace localRef to be rejected")
	}
	if !strings.Contains(err.Error(), "must match the DataguardBroker namespace") {
		t.Fatalf("expected localRef namespace validation error, got: %v", err)
	}
}

func TestDataguardBrokerWebhookValidateCreateRejectsTopologyShellBreakoutText(t *testing.T) {
	obj := validDataguardBrokerTopology()
	obj.Spec.Topology.Members[0].Endpoints[0].Host = "primary-a\n__CODex_EOF__\ntouch /tmp/dg-poc"

	_, err := (&DataguardBroker{}).ValidateCreate(context.Background(), obj)
	if err == nil {
		t.Fatalf("expected topology endpoint shell breakout text to be rejected")
	}
	if !strings.Contains(err.Error(), "must not contain control characters") {
		t.Fatalf("expected control character validation error, got: %v", err)
	}
}

func TestDataguardBrokerWebhookValidateCreateRejectsTopologyDescriptorHost(t *testing.T) {
	obj := validDataguardBrokerTopology()
	obj.Spec.Topology.Members[0].Endpoints[0].Host = "primary-a)(ADDRESS=(PROTOCOL=TCP)(HOST=evil)(PORT=1521"

	_, err := (&DataguardBroker{}).ValidateCreate(context.Background(), obj)
	if err == nil {
		t.Fatalf("expected topology endpoint descriptor host to be rejected")
	}
	if !strings.Contains(err.Error(), "without TNS descriptor punctuation") {
		t.Fatalf("expected host grammar validation error, got: %v", err)
	}
}

func TestDataguardBrokerWebhookValidateCreateRejectsTopologySSLServerDNBreakoutText(t *testing.T) {
	obj := validDataguardBrokerTopology()
	obj.Spec.Topology.Members[0].TCPS = &DataguardTCPSConfig{Enabled: true}
	obj.Spec.Topology.Members[0].Endpoints[0].Protocol = "TCPS"
	obj.Spec.Topology.Members[0].Endpoints[0].Port = 2484
	obj.Spec.Topology.Members[0].Endpoints[0].SSLServerDN = "CN=primary-a\n__CODex_EOF__\ntouch /tmp/dg-poc"

	_, err := (&DataguardBroker{}).ValidateCreate(context.Background(), obj)
	if err == nil {
		t.Fatalf("expected topology endpoint sslServerDN shell breakout text to be rejected")
	}
	if !strings.Contains(err.Error(), "must not contain control characters") {
		t.Fatalf("expected control character validation error, got: %v", err)
	}
...
func TestDataguardBrokerWebhookValidateCreateAllowsTopologyIPHosts(t *testing.T) {
	obj := validDataguardBrokerTopology()
	obj.Spec.Topology.Members[0].Endpoints[0].Host = "192.0.2.10"
	obj.Spec.Topology.Members[1].Endpoints[0].Host = "2001:db8::10"
	...
	_, err := (&DataguardBroker{}).ValidateCreate(context.Background(), obj)
	if err != nil {
		t.Fatalf("expected IPv4 and IPv6 topology hosts to validate, got: %v", err)
	}
}

func TestDataguardBrokerWebhookValidateCreateRejectsUnsafeDBUniqueName(t *testing.T) {
	obj := validDataguardBrokerTopology()
	obj.Spec.Topology.Members[0].DBUniqueName = "PRIMDB' scope=both sid='*'; shutdown immediate; --"

	_, err := (&DataguardBroker{}).ValidateCreate(context.Background(), obj)
	if err == nil {
		t.Fatalf("expected unsafe dbUniqueName to be rejected")
	}
	if !strings.Contains(err.Error(), "valid Oracle DB_UNIQUE_NAME") {
		t.Fatalf("expected dbUniqueName validation error, got: %v", err)
	}
}

func TestDataguardBrokerWebhookValidateCreateRejectsUnsafeServiceName(t *testing.T) {
	obj := validDataguardBrokerTopology()
	obj.Spec.Topology.Members[0].Endpoints[0].ServiceName = "PRIMDB)(ADDRESS=(PROTOCOL=TCP)(HOST=evil)(PORT=1521"

	_, err := (&DataguardBroker{}).ValidateCreate(context.Background(), obj)
	if err == nil {
		t.Fatalf("expected unsafe serviceName to be rejected")
	}
	if !strings.Contains(err.Error(), "serviceName") {
		t.Fatalf("expected serviceName validation error, got: %v", err)
	}
}

func TestDataguardBrokerWebhookValidateCreateAllowsDottedServiceName(t *testing.T) {
	obj := validDataguardBrokerTopology()
	obj.Spec.Topology.Members[0].Endpoints[0].ServiceName = "primdb.example.com"
	obj.Spec.Topology.Members[1].AdminSecretRef = &DataguardSecretRef{
		SecretName: "standby-admin",
		SecretKey:  "password",
	}
	...
	_, err := (&DataguardBroker{}).ValidateCreate(context.Background(), obj)
	if err != nil {
		t.Fatalf("expected dotted serviceName to validate, got: %v", err)
	}
}

func TestDataguardBrokerWebhookValidateCreateRequiresDBUniqueNameForUnsafeNameFallback(t *testing.T) {
	obj := validDataguardBrokerTopology()
	obj.Spec.Topology.Members[0].DBUniqueName = ""

	_, err := (&DataguardBroker{}).ValidateCreate(context.Background(), obj)
	if err == nil {
		t.Fatalf("expected member name fallback with hyphen to require dbUniqueName")
	}
	if !strings.Contains(err.Error(), "dbUniqueName is required when member.name is not a valid Oracle DB_UNIQUE_NAME") {
		t.Fatalf("expected dbUniqueName fallback validation error, got: %v", err)
	}
}
