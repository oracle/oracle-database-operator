...
func TestShardingWebhookDefaultRejectsOversizedShardNum(t *testing.T) {
	cr := &ShardingDatabase{
		Spec: ShardingDatabaseSpec{
			ShardInfo: []ShardingDetails{{
				ShardPreFixName: "sh",
				ShardNum:        int32(maxDesiredShardReplicas + 1),
			}},
		},
	}

	err := cr.Default(context.Background(), cr)
	if err == nil {
		t.Fatalf("expected oversized shardNum to be rejected")
	}
	if !strings.Contains(err.Error(), "desired shards") {
		t.Fatalf("expected desired shard count validation error, got: %v", err)
	}
}

func TestValidateShardInfoRejectsOversizedStandbyDerivedCount(t *testing.T) {
	cr := &ShardingDatabase{
		Spec: ShardingDatabaseSpec{
			ShardingType: "USER",
			ShardInfo: []ShardingDetails{{
				ShardPreFixName:   "stby",
				ShardSpaceDetails: &ShardSpaceSpec{Name: "ss1"},
				StandbyConfig: &StandbyConfig{
					StandbyPerPrimary: int32(maxDesiredShardReplicas + 1),
					PrimarySources: []StandbyPrimarySource{{
						ConnectString: "primary",
					}},
				},
			}},
		},
	...
	errs := cr.validateShardInfo()
	if len(errs) == 0 {
		t.Fatalf("expected oversized standby-derived shard count to be rejected")
	}
	if !strings.Contains(errs.ToAggregate().Error(), "desired shards") {
		t.Fatalf("expected desired shard count validation error, got: %v", errs)
	}
}

func TestShardingWebhookRejectsUnsafePrimarySourceConnectStrings(t *testing.T) {
	tests := []struct {
		name   string
		source StandbyPrimarySource
		want   string
	}{
		{
			name:   "direct connect string heredoc breakout",
			source: StandbyPrimarySource{ConnectString: "//primary:1521/PRIM_DGMGRL\nEOF\ntouch /tmp/dgmgrl_probe"},
			want:   "DGMGRL metacharacter",
		},
		{
			name: "details connect string quoted command",
			source: StandbyPrimarySource{Details: &PrimaryEndpointRef{
				ConnectString: "//primary:1521/PRIM_DGMGRL\"; show configuration; --",
			}},
			want: "DGMGRL metacharacter",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			errs := validatePrimarySourceList(
				field.NewPath("spec").Child("shardInfo").Index(0).Child("standbyConfig").Child("primarySources"),
				[]StandbyPrimarySource{tt.source},
				"default",
				false,
			)
			if len(errs) == 0 {
				t.Fatalf("expected validation error containing %q, got none", tt.want)
			}
			got := errs.ToAggregate().Error()
			if !strings.Contains(got, tt.want) {
				t.Fatalf("expected validation error containing %q, got %v", tt.want, got)
			}
		})
	}
}
...
func TestValidateShardInfoSystemPrimaryStandbyConstraints(t *testing.T) {
	tests := []struct {
		...
		{
			name: "system DG allows externalPrimarySources on standby shardInfo",
			spec: ShardingDatabaseSpec{
				ShardingType:    "SYSTEM",
				ReplicationType: "DG",
				ShardInfo: []ShardingDetails{
					{
						...
						ExternalPrimarySources: []StandbyPrimarySource{
							{Details: &PrimaryEndpointRef{Host: "pri1.example.com", Port: 1521, CdbName: "prim1"}},
							{Details: &PrimaryEndpointRef{Host: "pri2.example.com", Port: 1521, CdbName: "prim2"}},
						},
					},
				},
			},
		...
		{
			name: "system DG rejects externalPrimarySources on primary shardInfo",
			spec: ShardingDatabaseSpec{
				ShardingType:    "SYSTEM",
				ReplicationType: "DG",
				ShardInfo: []ShardingDetails{
			...
			wantErr: "must be declared on standby shardInfo entries",
		},
		{
		...
		{
			name: "user DG rejects externalPrimarySources",
			spec: ShardingDatabaseSpec{
				ShardingType:    "USER",
				ReplicationType: "DG",
				ShardInfo: []ShardingDetails{
			...
			wantErr: "should use standbyConfig.primarySources instead",
		},
		{
			name: "composite DG allows externalPrimarySources on standby shardInfo",
			spec: ShardingDatabaseSpec{
				ShardingType:    "COMPOSITE",
				ReplicationType: "DG",
				ShardInfo: []ShardingDetails{
...
func TestValidateShardingMemberServiceNamesRejectsInvalidName(t *testing.T) {
	cr := &ShardingDatabase{
		Spec: ShardingDatabaseSpec{
			Gsm: []GsmSpec{{Name: "Bad_Name"}},
		},
	}

	errList := cr.validateShardingMemberServiceNames()
	if len(errList) == 0 {
	...
	if !hasErrContaining(errs, "valid Kubernetes Service name") {
		t.Fatalf("expected Kubernetes Service name validation error, got: %v", errs)
	}
}

func TestValidateShardingMemberServiceNamesRejectsExternalCollision(t *testing.T) {
	cr := &ShardingDatabase{
		Spec: ShardingDatabaseSpec{
			IsExternalSvc: true,
			Gsm: []GsmSpec{
	...
	errList := cr.validateShardingMemberServiceNames()
	if len(errList) == 0 {
		t.Fatalf("expected validation error for derived external service name collision")
	}
	errs := make([]error, 0, len(errList))
	for _, e := range errList {
		errs = append(errs, e)
	...
	if !hasErrContaining(errs, "collides") {
		t.Fatalf("expected service-name collision error, got: %v", errs)
	}
}
...
func TestMergeDesiredAndExistingShardsPreservesExistingPrimaryDatabaseRefWhenDesiredIsEmpty(t *testing.T) {
	existing := []ShardSpec{{
		Name:     "std1",
		DeployAs: "STANDBY",
		PrimaryDatabaseRef: &DatabaseRef{
			Host:    "pri1.example.com",
			Port:    1521,
			CdbName: "PRIM1",
			PdbName: "PRIM1PDB",
		},
	...
	desired := []ShardSpec{{
		Name:               "std1",
		DeployAs:           "STANDBY",
		PrimaryDatabaseRef: nil,
	}}

	merged := mergeDesiredAndExistingShards(existing, desired)
	if len(merged) != 1 {
		t.Fatalf("expected one merged shard, got %d", len(merged))
	}
	if merged[0].PrimaryDatabaseRef == nil {
		t.Fatalf("expected existing primaryDatabaseRef to be preserved")
	}
	if merged[0].PrimaryDatabaseRef.Host != "pri1.example.com" {
		t.Fatalf("expected preserved host pri1.example.com, got %#v", merged[0].PrimaryDatabaseRef)
	}
}
