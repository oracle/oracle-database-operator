...
const (
	...
	maxDesiredShardReplicas int64 = 1024
...
func (r *ShardingDatabase) Default(ctx context.Context, obj *ShardingDatabase) error {
	...
	totalShard, countErrs := cr.desiredShardReplicaTotal()
	if len(countErrs) > 0 {
		return countErrs.ToAggregate()
	}
...
func (r *ShardingDatabase) ValidateCreate(ctx context.Context, obj *ShardingDatabase) (admission.Warnings, error) {
	...
	validationErrs1 = cr.validateShardingMemberServiceNames()
	if validationErrs1 != nil {
		validationErr = append(validationErr, validationErrs1...)
	}
...
func (r *ShardingDatabase) ValidateUpdate(ctx context.Context, oldObj, newObj *ShardingDatabase) (admission.Warnings, error) {
	...
	validationErrs1 = newCR.validateShardingMemberServiceNames()
	if validationErrs1 != nil {
		validationErr = append(validationErr, validationErrs1...)
	}
...
type serviceNameClaim struct {
	path   *field.Path
	source string
}

func (r *ShardingDatabase) validateShardingMemberServiceNames() field.ErrorList {
	var validationErrs field.ErrorList
	claims := make(map[string]serviceNameClaim)

	register := func(name string, path *field.Path, source string) {
		if previous, ok := claims[name]; ok {
			validationErrs = append(validationErrs,
				field.Invalid(path, name,
					fmt.Sprintf("service name collides with %s at %s", previous.source, previous.path.String())))
			return
		}
		claims[name] = serviceNameClaim{path: path, source: source}
	}

	validateMemberName := func(name string, path *field.Path, source string) {
		name = strings.TrimSpace(name)
		if name == "" {
			validationErrs = append(validationErrs, field.Required(path, source+" name must be set"))
			return
		}
		if msgs := validationutil.IsDNS1123Label(name); len(msgs) > 0 {
			validationErrs = append(validationErrs,
				field.Invalid(path, name, "must be a valid Kubernetes Service name: "+strings.Join(msgs, "; ")))
			return
		}

		register(name, path, source+" local Service")

		if !r.Spec.IsExternalSvc {
			return
		}

		externalName := name + "0-svc"
		if msgs := validationutil.IsDNS1123Label(externalName); len(msgs) > 0 {
			validationErrs = append(validationErrs,
				field.Invalid(path, name,
					fmt.Sprintf("derived external Service name %q must be a valid Kubernetes Service name: %s",
						externalName, strings.Join(msgs, "; "))))
			return
		}
		register(externalName, path, source+" external Service")
	}

	for i := range r.Spec.Catalog {
		validateMemberName(r.Spec.Catalog[i].Name, field.NewPath("spec").Child("catalog").Index(i).Child("name"), "catalog")
	}
	for i := range r.Spec.Gsm {
		validateMemberName(r.Spec.Gsm[i].Name, field.NewPath("spec").Child("gsm").Index(i).Child("name"), "gsm")
	}
	for i := range r.Spec.Shard {
		validateMemberName(r.Spec.Shard[i].Name, field.NewPath("spec").Child("shard").Index(i).Child("name"), "shard")
	}
...
func validateStorageSource(source *StorageSource, path *field.Path) field.ErrorList {
	if source == nil {
		return nil
	}
	if source.OCI == nil {
		return field.ErrorList{field.Required(path.Child("oci"), "oci storage source must be specified")}
	}
	if strings.TrimSpace(source.OCI.VolumeBackupID) == "" {
		return field.ErrorList{field.Required(path.Child("oci").Child("volumeBackupId"), "volumeBackupId must be set")}
	}
	return nil
}
...
func (r *ShardingDatabase) validateAdditionalPVCConfig() field.ErrorList {
	...
	for i := range r.Spec.ShardInfo {
		validationErrs = append(validationErrs, validateStorageSource(r.Spec.ShardInfo[i].StorageSource, field.NewPath("spec").Child("shardInfo").Index(i).Child("storageSource"))...)
		if len(r.Spec.ShardInfo[i].Nodes) > 0 && len(r.Spec.ShardInfo[i].NodeSelector) > 0 {
			validationErrs = append(validationErrs, field.Forbidden(
				field.NewPath("spec").Child("shardInfo").Index(i),
				"nodes and nodeSelector are mutually exclusive; specify only one",
			))
		}
	}

	for i := range r.Spec.Shard {
		validationErrs = append(validationErrs, validateStorageSource(r.Spec.Shard[i].StorageSource, field.NewPath("spec").Child("shard").Index(i).Child("storageSource"))...)
		if len(r.Spec.Shard[i].Nodes) > 0 && len(r.Spec.Shard[i].NodeSelector) > 0 {
			validationErrs = append(validationErrs, field.Forbidden(
				field.NewPath("spec").Child("shard").Index(i),
				"nodes and nodeSelector are mutually exclusive; specify only one",
			))
		}
...
func (r *ShardingDatabase) validateShardInfo() field.ErrorList {
	var validationErrs field.ErrorList
	if countErrs := r.validateDesiredShardReplicaCounts(); len(countErrs) > 0 {
		return countErrs
	}
	var replicas int32
	...
	sysExternalStandbyReplicaCountByGroup := map[string]int32{}
	...
	compositeExternalReplicaCountBySpaceGroup := map[string]map[string]int32{}
	...
	for pindex := range r.Spec.ShardInfo {
		...
		if errs := validateExternalPrimarySourceExclusive(r, pindex); len(errs) > 0 {
			validationErrs = append(validationErrs, errs...)
		}
		...
		if standbyConfigPrimaryCount(r.Spec.ShardInfo[pindex].StandbyConfig) > 0 &&
			externalPrimarySourceCount(r.Spec.ShardInfo[pindex].ExternalPrimarySources, r.Namespace) > 0 {
			validationErrs = append(validationErrs,
				field.Forbidden(field.NewPath("spec").Child("shardInfo").Index(pindex),
					"standbyConfig.primarySources and externalPrimarySources are mutually exclusive"))
		}
			...
			if modeHint == modeSystem {
				...
				if groupKey != "" {
					...
					case "PRIMARY":
						...
						if externalPrimarySourceCount(r.Spec.ShardInfo[pindex].ExternalPrimarySources, r.Namespace) > 0 {
							validationErrs = append(validationErrs,
								field.Forbidden(field.NewPath("spec").Child("shardInfo").Index(pindex).Child("externalPrimarySources"),
									"system sharding externalPrimarySources must be declared on standby shardInfo entries"))
						}
					case "STANDBY", "ACTIVE_STANDBY":
						...
						if externalCount := externalPrimarySourceCount(r.Spec.ShardInfo[pindex].ExternalPrimarySources, r.Namespace); externalCount > 0 {
							sysExternalStandbyReplicaCountByGroup[groupKey] += externalCount
							if externalCount != replicaCount {
								validationErrs = append(validationErrs,
									field.Invalid(field.NewPath("spec").Child("shardInfo").Index(pindex).Child("externalPrimarySources"),
										externalCount,
										fmt.Sprintf("system sharding requires one externalPrimarySources entry per standby replica; expected %d", replicaCount)))
							}
						}
					}
				}
			...
			if modeHint == modeUser && replType == replDG {
				...
				if spaceKey != "" {
					...
					if externalPrimarySourceCount(r.Spec.ShardInfo[pindex].ExternalPrimarySources, r.Namespace) > 0 {
						validationErrs = append(validationErrs,
							field.Forbidden(field.NewPath("spec").Child("shardInfo").Index(pindex).Child("externalPrimarySources"),
								"user sharding should use standbyConfig.primarySources instead of externalPrimarySources"))
					}
					deployAs := strings.ToUpper(strings.TrimSpace(ss.DeployAs))
					if deployAs == "" {
						ss.DeployAs = "STANDBY"
		...
		case hasGroup && hasSpace:
			...
			if modeHint == modeComposite {
				...
				if _, ok := compositeExternalReplicaCountBySpaceGroup[spaceKey]; !ok {
					compositeExternalReplicaCountBySpaceGroup[spaceKey] = map[string]int32{}
				}
				switch deployAs {
				case "PRIMARY":
					...
					if externalPrimarySourceCount(r.Spec.ShardInfo[pindex].ExternalPrimarySources, r.Namespace) > 0 {
						validationErrs = append(validationErrs,
							field.Forbidden(field.NewPath("spec").Child("shardInfo").Index(pindex).Child("externalPrimarySources"),
								"composite sharding externalPrimarySources must be declared on standby shardInfo entries"))
					}
				case "STANDBY", "ACTIVE_STANDBY":
					compositeStandbyReplicaCountBySpaceGroup[spaceKey][groupKey] += replicaCount
					if externalCount := externalPrimarySourceCount(r.Spec.ShardInfo[pindex].ExternalPrimarySources, r.Namespace); externalCount > 0 {
						compositeExternalReplicaCountBySpaceGroup[spaceKey][groupKey] += externalCount
						if externalCount != replicaCount {
							validationErrs = append(validationErrs,
								field.Invalid(field.NewPath("spec").Child("shardInfo").Index(pindex).Child("externalPrimarySources"),
									externalCount,
									fmt.Sprintf("composite sharding requires one externalPrimarySources entry per standby replica; expected %d", replicaCount)))
						}
					}
				}
			}
	...
	if replType == replDG && typeCounts.groupOnly > 0 && typeCounts.spaceOnly == 0 && typeCounts.both == 0 {
		if modeHint == modeSystem {
			...
			if len(sysPrimaryReplicaCountByGroup) == 0 {
				var totalStandbyReplicas int32
				var totalExternalReplicas int32
				for _, standbyCount := range sysStandbyReplicaCountByGroup {
					totalStandbyReplicas += standbyCount
				}
				for _, externalCount := range sysExternalStandbyReplicaCountByGroup {
					totalExternalReplicas += externalCount
				}
				if totalStandbyReplicas == 0 || totalExternalReplicas != totalStandbyReplicas {
					validationErrs = append(validationErrs,
						field.Invalid(field.NewPath("spec").Child("shardInfo").Child("externalPrimarySources"),
							totalExternalReplicas,
							"System sharding standby-only deployments must provide externalPrimarySources for every standby shard"))
				}
			}
			if len(sysPrimaryReplicaCountByGroup) == 1 {
		...
		if modeHint == modeComposite {
			for spaceKey, primaryGroups := range compositePrimaryGroupsBySpace {
				if len(primaryGroups) == 0 {
					var totalStandbyReplicas int32
					var totalExternalReplicas int32
					for _, standbyCount := range compositeStandbyReplicaCountBySpaceGroup[spaceKey] {
						totalStandbyReplicas += standbyCount
					}
					for _, externalCount := range compositeExternalReplicaCountBySpaceGroup[spaceKey] {
						totalExternalReplicas += externalCount
					}
					if totalStandbyReplicas == 0 || totalExternalReplicas != totalStandbyReplicas {
						validationErrs = append(validationErrs,
							field.Invalid(field.NewPath("spec").Child("shardInfo").Child("shardSpaceDetails").Child("name"),
								spaceKey,
								"Composite sharding standby-only deployments must provide externalPrimarySources for every standby shard in the shardSpace"))
					}
					continue
				}
...
func validateStandbyConfigPrimarySourceExclusive(r *ShardingDatabase, index int) field.ErrorList {
	...
	return validatePrimarySourceList(
		field.NewPath("spec").Child("shardInfo").Index(index).Child("standbyConfig").Child("primarySources"),
		cfg.PrimarySources,
		r.Namespace,
		false,
	)
...
func validateExternalPrimarySourceExclusive(r *ShardingDatabase, index int) field.ErrorList {
	if r == nil || index < 0 || index >= len(r.Spec.ShardInfo) {
		return nil
	}
	return validatePrimarySourceList(
		field.NewPath("spec").Child("shardInfo").Index(index).Child("externalPrimarySources"),
		r.Spec.ShardInfo[index].ExternalPrimarySources,
		r.Namespace,
		true,
	)
}

func validatePrimarySourceList(path *field.Path, sources []StandbyPrimarySource, defaultNamespace string, requireStructuredDetails bool) field.ErrorList {
	var errs field.ErrorList
	...
	for i := range sources {
		...
		if connect := strings.TrimSpace(source.ConnectString); connect != "" {
			selected++
			if err := validateDgmgrlConnectString(connect); err != nil {
				errs = append(errs, field.Invalid(sourcePath.Child("connectString"), source.ConnectString, err.Error()))
			}
		}
		...
		if requireStructuredDetails && source.Details == nil {
			errs = append(errs, field.Required(sourcePath.Child("details"), "details is required for externalPrimarySources"))
		}
		if details := source.Details; details != nil {
			if connect := strings.TrimSpace(details.ConnectString); connect != "" {
				if err := validateDgmgrlConnectString(connect); err != nil {
					errs = append(errs, field.Invalid(sourcePath.Child("details").Child("connectString"), details.ConnectString, err.Error()))
				}
			}
			if requireStructuredDetails {
				if strings.TrimSpace(details.Host) == "" {
					errs = append(errs, field.Required(sourcePath.Child("details").Child("host"), "host is required for externalPrimarySources"))
				}
				if strings.TrimSpace(details.CdbName) == "" {
					errs = append(errs, field.Required(sourcePath.Child("details").Child("cdbName"), "cdbName is required for externalPrimarySources"))
				}
			} else if strings.TrimSpace(details.ConnectString) == "" {
				if strings.TrimSpace(details.Host) == "" {
		...
		if key := standbyPrimarySourceIdentityKey(source, defaultNamespace); key != "" {
			if seen[key] {
	...
	for pindex := range r.Spec.ShardInfo {
		...
		for i := 0; i < int(replicas); i++ {
			...
			*(r.Spec.Shard[shardIndex].ImagePulllPolicy) = corev1.PullPolicy("Always")
			...
			if len(r.Spec.Shard[shardIndex].NodeSelector) == 0 && len(r.Spec.ShardInfo[pindex].NodeSelector) > 0 {
				r.Spec.Shard[shardIndex].NodeSelector = make(map[string]string, len(r.Spec.ShardInfo[pindex].NodeSelector))
				for key, value := range r.Spec.ShardInfo[pindex].NodeSelector {
					r.Spec.Shard[shardIndex].NodeSelector[key] = value
				}
			}
			if r.Spec.Shard[shardIndex].StorageSource == nil && r.Spec.ShardInfo[pindex].StorageSource != nil {
				r.Spec.Shard[shardIndex].StorageSource = r.Spec.ShardInfo[pindex].StorageSource.DeepCopy()
			}
...
func (r *ShardingDatabase) buildDesiredShardSpec() []ShardSpec {
	...
	totalShard, countErrs := tmp.desiredShardReplicaTotal()
	if len(countErrs) > 0 {
		return nil
	}
...
func mergeDesiredAndExistingShards(existing []ShardSpec, desired []ShardSpec) []ShardSpec {
	...
	for _, d := range desired {
		...
		if old, ok := existingByName[name]; ok {
			...
			// Preserve controller-derived shard primary refs unless shardInfo
			// explicitly rebuilds a populated value for this shard.
			if d.PrimaryDatabaseRef != nil && strings.TrimSpace(d.PrimaryDatabaseRef.Host) != "" {
...
func (r *ShardingDatabase) validateDesiredShardReplicaCounts() field.ErrorList {
	_, errs := r.desiredShardReplicaTotal()
	return errs
}

func (r *ShardingDatabase) desiredShardReplicaTotal() (int32, field.ErrorList) {
	var allErrs field.ErrorList
	var total int64
	modeHint := normalizeShardingType(&r.Spec)
	for i := range r.Spec.ShardInfo {
		count, countPath := desiredShardInfoReplicaCount(modeHint, &r.Spec.ShardInfo[i], field.NewPath("spec").Child("shardInfo").Index(i))
		if count == 0 {
			if modeHint == modeUser {
				count = 1
			} else {
			...
			countPath = field.NewPath("spec").Child("shardInfo").Index(i).Child("shardNum")
		}
		if count > maxDesiredShardReplicas {
			allErrs = append(allErrs, field.Invalid(countPath, count, fmt.Sprintf("must not resolve to more than %d desired shards", maxDesiredShardReplicas)))
		}
		total += count
		if total > maxDesiredShardReplicas {
			allErrs = append(allErrs, field.Invalid(field.NewPath("spec").Child("shardInfo"), total, fmt.Sprintf("total desired shards must not exceed %d", maxDesiredShardReplicas)))
			break
		}
	}
	if len(allErrs) > 0 {
...
func desiredShardInfoReplicaCount(mode shardingMode, info *ShardingDetails, infoPath *field.Path) (int64, *field.Path) {
	if info == nil {
		return 0, infoPath
	}
	if mode == modeUser {
		if c := standbyConfigDerivedShardCount64(info.StandbyConfig); c > 0 {
			return c, infoPath.Child("standbyConfig").Child("standbyPerPrimary")
		}
	}
	if info.ShardNum > 0 {
		return int64(info.ShardNum), infoPath.Child("shardNum")
	}
	if info.Replicas > 0 {
		return int64(info.Replicas), infoPath.Child("replicas")
	}
	return 0, infoPath.Child("shardNum")
}

func standbyConfigDerivedShardCount64(cfg *StandbyConfig) int64 {
	if cfg == nil {
		return 0
	}
	primaryCount := int64(standbyConfigPrimaryCount(cfg))
	if primaryCount == 0 {
		return 0
	}
	perPrimary := int64(cfg.StandbyPerPrimary)
	if perPrimary <= 0 {
		perPrimary = 1
	}
...
func countTotalReplicas(shardInfo []ShardingDetails) int32 {
	tmp := &ShardingDatabase{Spec: ShardingDatabaseSpec{ShardInfo: shardInfo}}
	total, errs := tmp.desiredShardReplicaTotal()
	if len(errs) > 0 {
		return 0
	}
	return total
...
func externalPrimarySourceCount(sources []StandbyPrimarySource, defaultNamespace string) int32 {
	seen := map[string]bool{}
	var count int32
	for i := range sources {
		key := standbyPrimarySourceIdentityKey(&sources[i], defaultNamespace)
		if key == "" || seen[key] {
			continue
		}
		seen[key] = true
...
func validateDgmgrlConnectString(connect string) error {
	...
	for _, r := range connect {
		switch r {
		case '\n', '\r', '"', ';':
			return fmt.Errorf("must not contain DGMGRL metacharacter %q", r)
		}
		if r == '\'' {
			return fmt.Errorf("must not contain DGMGRL metacharacter %q", r)
		}
...
type shardingDataguardShardInfoProjection struct {
	...
	PrimaryDatabaseRef     *DatabaseRef
	StandbyConfig          *StandbyConfig
	ExternalPrimarySources []StandbyPrimarySource
}
	...
	if len(cr.Spec.ShardInfo) > 0 {
		...
		for i := range cr.Spec.ShardInfo {
			...
			projection.ShardInfo = append(projection.ShardInfo, shardingDataguardShardInfoProjection{
				...
				ExternalPrimarySources: info.ExternalPrimarySources,
			})
		}
	}
