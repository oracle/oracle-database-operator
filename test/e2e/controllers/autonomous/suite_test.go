...
var _ = BeforeSuite(func() {
	...
	**************************************************/
	...
	By("Creating a k8s secret to hold admin password", func() {
		...
		Expect(err).ToNot(HaveOccurred())
		Expect(k8sClient.Create(e2eCtx, adminSecret)).To(Succeed())
	})
	...
	By("Creating a k8s secret to hold new admin password", func() {
		...
		Expect(err).ToNot(HaveOccurred())
		Expect(k8sClient.Create(e2eCtx, newAdminSecret)).To(Succeed())
	})
	...
	By("Creating a k8s secret to hold wallet password", func() {
		...
		Expect(err).ToNot(HaveOccurred())
		Expect(k8sClient.Create(e2eCtx, walletSecret)).To(Succeed())
	})
})
...
var _ = AfterSuite(func() {
	...
	*/
	...
	k8sClient.List(e2eCtx, adbList, options)
	...
	for _, adb := range adbList.Items {
		if adb.Spec.Details.Id != nil {
			...
			Expect(e2eutil.DeleteAutonomousDatabase(e2eCtx, dbClient, adb.Spec.Details.Id)).Should(Succeed())
		}
	}
