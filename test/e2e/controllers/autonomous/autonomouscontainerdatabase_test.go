...
var _ = Describe("test ACD binding", func() {
	...
	Describe("ACD Provisioning", func() {
		It("Should create an AutonomousContainerDatabase resource and in OCI", func() {
			...
			Expect(k8sClient.Create(e2eCtx, provisionAcd)).Should(Succeed())
		})

		It("Should check ACD status is BACKUP IN PROGRESS", e2ebehavior.AssertACDState(e2eCtx, &k8sClient, &dbClient, &acdLookupKey, database.AutonomousContainerDatabaseLifecycleStateBackupInProgress, time.Minute*35))

		It("Should check ACD status is AVAILABLE", e2ebehavior.AssertACDState(e2eCtx, &k8sClient, &dbClient, &acdLookupKey, database.AutonomousContainerDatabaseLifecycleStateAvailable, time.Minute*60))

		It("Should save ACD ocid for next test", func() {
			acd := &dbv1alpha1.AutonomousContainerDatabase{}
			Expect(k8sClient.Get(e2eCtx, acdLookupKey, acd)).To(Succeed())
			acdID = *acd.Spec.AutonomousContainerDatabaseOCID
		})

		It("Should delete ACD local resource", e2ebehavior.AssertACDLocalDelete(e2eCtx, &k8sClient, &dbClient, &acdLookupKey))
	})
	...
	Describe("ACD Binding", func() {
		It("Should create an AutonomousContainerDatabase resource", func() {
			...
			Expect(k8sClient.Create(e2eCtx, acd)).Should(Succeed())
		})

		It("Should bind to an ACD", e2ebehavior.AssertACDBind(e2eCtx, &k8sClient, &dbClient, &acdLookupKey, database.AutonomousContainerDatabaseLifecycleStateAvailable))

		It("Should update the ACD", e2ebehavior.UpdateAndAssertACDSpec(e2eCtx, &k8sClient, &dbClient, &acdLookupKey))

		It("Should restart the ACD", e2ebehavior.AssertACDRestart(e2eCtx, &k8sClient, &dbClient, &acdLookupKey))

		It("Should terminate the ACD", e2ebehavior.AssertACDTerminate(e2eCtx, &k8sClient, &dbClient, &acdLookupKey))
	})
})
