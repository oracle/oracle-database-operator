...
func AssertProvision(ctx context.Context, k8sClient *client.Client, adbLookupKey *types.NamespacedName) func() {
	return func() {
		...
		Eventually(func() (*string, error) {
			err := derefK8sClient.Get(ctx, *adbLookupKey, createdADB)
			if err != nil {
				return nil, err
			}
...
func AssertBind(ctx context.Context, k8sClient *client.Client, adbLookupKey *types.NamespacedName) func() {
	return func() {
		Expect(k8sClient).NotTo(BeNil())
		Expect(adbLookupKey).NotTo(BeNil())
		...
		*/
		...
		Eventually(func() bool {
			err := derefK8sClient.Get(ctx, *adbLookupKey, boundADB)
			if err != nil {
				return false
			}
			return (boundADB.Spec.Details.CompartmentId != nil &&
...
func AssertWallet(ctx context.Context, k8sClient *client.Client, adbLookupKey *types.NamespacedName) func() {
	return func() {
		...
		Expect(derefK8sClient.Get(ctx, *adbLookupKey, adb)).To(Succeed())
		...
		Eventually(func() bool {
			err := derefK8sClient.Get(ctx, walletLookupKey, instanceWallet)
			return err == nil
...
func UpdateDetails(ctx context.Context, k8sClient *client.Client, dbClient *database.DatabaseClient, adbLookupKey *types.NamespacedName, newSecretName string, newAdminPassword *string) func() *dbv1alpha1.AutonomousDatabase {
	return func() *dbv1alpha1.AutonomousDatabase {
		...
		Expect(derefK8sClient.Get(ctx, *adbLookupKey, expectedADB)).To(Succeed())
		...
		Eventually(func() (database.AutonomousDatabaseLifecycleStateEnum, error) {
			listResp, err := e2eutil.ListAutonomousDatabases(ctx, derefDBClient, expectedADB.Spec.Details.CompartmentId, expectedADB.Spec.Details.DisplayName)
			if err != nil {
				return "", err
			}
		...
		Expect(derefK8sClient.Update(ctx, expectedADB)).To(Succeed())
...
func AssertADBDetails(ctx context.Context, k8sClient *client.Client,
	...
	return func() {
		...
		Eventually(func() (bool, error) {
			...
			resp, err := e2eutil.GetAutonomousDatabase(ctx, derefDBClient, expectedADB.Spec.Details.Id, &retryPolicy)
			if err != nil {
				return false, err
			}
			...
			same := compareString(expectedADBDetails.Id, resp.AutonomousDatabase.Id) &&
				compareString(expectedADBDetails.CompartmentId, resp.AutonomousDatabase.CompartmentId) &&
				...
				compareString(expectedADBDetails.SubnetId, resp.AutonomousDatabase.SubnetId) &&
				reflect.DeepEqual(expectedADBDetails.NsgIds, resp.AutonomousDatabase.NsgIds) &&
		...
		AssertADBLocalState(ctx, k8sClient, adbLookupKey, database.AutonomousDatabaseLifecycleStateAvailable)()
	}
}

func TestNetworkAccessRestricted(ctx context.Context, k8sClient *client.Client, dbClient *database.DatabaseClient, adbLookupKey *types.NamespacedName, acl []string, isMTLSConnectionRequired bool) func() {
	return func() {
		TestNetworkAccess(ctx, k8sClient, dbClient, adbLookupKey, nil, nil, acl, isMTLSConnectionRequired)()
	}
}

/* Runs a script that connects to an ADB */
func AssertAdminPassword(ctx context.Context, dbClient *database.DatabaseClient, databaseOCID *string, tnsEntry *string, adminPassword *string, walletPassword *string) error {
	By("Downloading wallet zip")
	walletZip, err := e2eutil.DownloadWalletZip(ctx, *dbClient, databaseOCID, walletPassword)
	if err != nil {
...
func TestNetworkAccessPrivate(ctx context.Context, k8sClient *client.Client, dbClient *database.DatabaseClient, adbLookupKey *types.NamespacedName, isMTLSConnectionRequired bool, subnetOCID *string, nsgOCIDs *string) func() {
	return func() {
		...
		adb := &dbv1alpha1.AutonomousDatabase{}
		derefK8sClient := *k8sClient
		Expect(derefK8sClient.Get(ctx, *adbLookupKey, adb)).Should(Succeed())

		TestNetworkAccess(ctx, k8sClient, dbClient, adbLookupKey, subnetOCID, nsgOCIDs, nil, isMTLSConnectionRequired)()
	}
}

func TestNetworkAccessPublic(ctx context.Context, k8sClient *client.Client, dbClient *database.DatabaseClient, adbLookupKey *types.NamespacedName) func() {
	return func() {
		TestNetworkAccess(ctx, k8sClient, dbClient, adbLookupKey, nil, nil, nil, true)()
	}
}

func TestNetworkAccess(ctx context.Context, k8sClient *client.Client, dbClient *database.DatabaseClient, adbLookupKey *types.NamespacedName, subnetOCID *string, nsgOCIDs *string, acl []string, isMTLSConnectionRequired bool) func() {
	return func() {
		Expect(k8sClient).NotTo(BeNil())
		Expect(dbClient).NotTo(BeNil())
		Expect(adbLookupKey).NotTo(BeNil())
		...
		adb := &dbv1alpha1.AutonomousDatabase{}
		AssertADBState(ctx, k8sClient, dbClient, adbLookupKey, database.AutonomousDatabaseLifecycleStateAvailable)()
		Expect(derefK8sClient.Get(ctx, *adbLookupKey, adb)).To(Succeed())

		adb.Spec.Details.SubnetId = subnetOCID
		adb.Spec.Details.NsgIds = []string{*nsgOCIDs}
		adb.Spec.Details.WhitelistedIps = acl
		adb.Spec.Details.IsMtlsConnectionRequired = common.Bool(isMTLSConnectionRequired)
		Expect(derefK8sClient.Update(ctx, adb)).To(Succeed())
		AssertADBDetails(ctx, k8sClient, dbClient, adbLookupKey, adb)()
	}
}
...
func UpdateAndAssertDetails(ctx context.Context, k8sClient *client.Client, dbClient *database.DatabaseClient, adbLookupKey *types.NamespacedName, newSecretName string, newAdminPassword *string, walletPassword *string) func() {
	return func() {
		expectedADB := UpdateDetails(ctx, k8sClient, dbClient, adbLookupKey, newSecretName, newAdminPassword)()
		AssertADBDetails(ctx, k8sClient, dbClient, adbLookupKey, expectedADB)()

		ocid := expectedADB.Spec.Details.Id
		tnsEntry := *expectedADB.Spec.Details.DbName + "_high"
		err := AssertAdminPassword(ctx, dbClient, ocid, &tnsEntry, newAdminPassword, walletPassword)
		Expect(err).ShouldNot(HaveOccurred())
	}
}

// UpdateAndAssertADBState updates adb state and then asserts if change is propagated to OCI
func UpdateAndAssertADBState(ctx context.Context, k8sClient *client.Client, dbClient *database.DatabaseClient, adbLookupKey *types.NamespacedName, state database.AutonomousDatabaseLifecycleStateEnum) func() {
	return func() {
		UpdateState(ctx, k8sClient, adbLookupKey, state)()
		AssertADBState(ctx, k8sClient, dbClient, adbLookupKey, state)()
	}
}

// AssertADBState asserts local and remote state
func AssertADBState(ctx context.Context, k8sClient *client.Client, dbClient *database.DatabaseClient, adbLookupKey *types.NamespacedName, state database.AutonomousDatabaseLifecycleStateEnum) func() {
	return func() {
		// Waits longer for the local resource to reach the desired state
		AssertADBLocalState(ctx, k8sClient, adbLookupKey, state)()

		// Double-check the state of the DB in OCI so the timeout can be shorter
		AssertADBRemoteState(ctx, k8sClient, dbClient, adbLookupKey, state)()
	}
}

// AssertHardLinkDelete asserts the database is terminated in OCI when hardLink is set to true
func AssertHardLinkDelete(ctx context.Context, k8sClient *client.Client, dbClient *database.DatabaseClient, adbLookupKey *types.NamespacedName) func() {
	return func() {
		Expect(k8sClient).NotTo(BeNil())
		Expect(dbClient).NotTo(BeNil())
		Expect(adbLookupKey).NotTo(BeNil())
		...
		adb := &dbv1alpha1.AutonomousDatabase{}
		Expect(derefK8sClient.Get(ctx, *adbLookupKey, adb)).To(Succeed())
		Expect(derefK8sClient.Delete(ctx, adb)).To(Succeed())
		...
		Eventually(func() (database.AutonomousDatabaseLifecycleStateEnum, error) {
			...
			return returnADBRemoteState(ctx, derefK8sClient, derefDBClient, adb.Spec.Details.Id, &retryPolicy)
		}, changeTimeout).Should(Equal(database.AutonomousDatabaseLifecycleStateTerminating))

		AssertSoftLinkDelete(ctx, k8sClient, adbLookupKey)()
	}
}

// AssertSoftLinkDelete asserts the database remains in OCI when hardLink is set to false
func AssertSoftLinkDelete(ctx context.Context, k8sClient *client.Client, adbLookupKey *types.NamespacedName) func() {
	return func() {
		Expect(k8sClient).NotTo(BeNil())
		Expect(adbLookupKey).NotTo(BeNil())
		...
		Expect(derefK8sClient.Get(ctx, *adbLookupKey, existingAdb)).To(Succeed())
		Expect(derefK8sClient.Delete(ctx, existingAdb)).To(Succeed())

		By("Checking if the AutonomousDatabase resource is deleted")
		Eventually(func() (isDeleted bool) {
			adb := &dbv1alpha1.AutonomousDatabase{}
			isDeleted = false
			err := derefK8sClient.Get(ctx, *adbLookupKey, adb)
...
func AssertADBLocalState(ctx context.Context, k8sClient *client.Client, adbLookupKey *types.NamespacedName, state database.AutonomousDatabaseLifecycleStateEnum) func() {
	return func() {
		Expect(k8sClient).NotTo(BeNil())
		Expect(adbLookupKey).NotTo(BeNil())
		...
		Eventually(func() (database.AutonomousDatabaseLifecycleStateEnum, error) {
			return returnADBLocalState(ctx, derefK8sClient, *adbLookupKey)
		}, changeLocalStateTimeout).Should(Equal(state))
	}
}

func AssertADBRemoteState(ctx context.Context, k8sClient *client.Client, dbClient *database.DatabaseClient, adbLookupKey *types.NamespacedName, state database.AutonomousDatabaseLifecycleStateEnum) func() {
	return func() {
		Expect(k8sClient).NotTo(BeNil())
		Expect(dbClient).NotTo(BeNil())
		Expect(adbLookupKey).NotTo(BeNil())
		...
		AssertADBRemoteStateOCID(ctx, k8sClient, dbClient, adb.Spec.Details.Id, state, changeTimeout)()
	}
}

// Backup takes ~15 minutes to complete, this function waits 20 minutes until ADB state is AVAILABLE
func AssertADBRemoteStateForBackupRestore(ctx context.Context, k8sClient *client.Client, dbClient *database.DatabaseClient, adbLookupKey *types.NamespacedName, state database.AutonomousDatabaseLifecycleStateEnum) func() {
	return func() {
		Expect(k8sClient).NotTo(BeNil())
		Expect(dbClient).NotTo(BeNil())
		Expect(adbLookupKey).NotTo(BeNil())
		...
		Expect(derefK8sClient.Get(ctx, *adbLookupKey, adb)).To(Succeed())
		By("Checking if the lifecycleState of remote resource is " + string(state))
		AssertADBRemoteStateOCID(ctx, k8sClient, dbClient, adb.Spec.Details.Id, state, backupTimeout)()
	}
}

func AssertADBRemoteStateOCID(ctx context.Context, k8sClient *client.Client, dbClient *database.DatabaseClient, adbID *string, state database.AutonomousDatabaseLifecycleStateEnum, timeout time.Duration) func() {
	return func() {
		Expect(k8sClient).NotTo(BeNil())
		Expect(dbClient).NotTo(BeNil())
		...
		Eventually(func() (database.AutonomousDatabaseLifecycleStateEnum, error) {
			return returnADBRemoteState(ctx, derefK8sClient, derefDBClient, adbID, nil)
		}, timeout, intervalTime).Should(Equal(state))
	}
}

// UpdateState updates state from local resource and OCI
func UpdateState(ctx context.Context, k8sClient *client.Client, adbLookupKey *types.NamespacedName, state database.AutonomousDatabaseLifecycleStateEnum) func() {
	return func() {
		Expect(k8sClient).NotTo(BeNil())
		Expect(adbLookupKey).NotTo(BeNil())
		...
		case database.AutonomousDatabaseLifecycleStateAvailable:
			adb.Spec.Action = "Start"
		case database.AutonomousDatabaseLifecycleStateStopped:
			adb.Spec.Action = "Stop"
		case database.AutonomousDatabaseLifecycleStateTerminated:
			adb.Spec.Action = "Terminate"
		}
		Expect(derefK8sClient.Update(ctx, adb)).To(Succeed())
	}
}

func returnADBLocalState(ctx context.Context, k8sClient client.Client, adbLookupKey types.NamespacedName) (database.AutonomousDatabaseLifecycleStateEnum, error) {
	adb := &dbv1alpha1.AutonomousDatabase{}
	err := k8sClient.Get(ctx, adbLookupKey, adb)
	if err != nil {
		return "", err
	}
...
func returnADBRemoteState(ctx context.Context, k8sClient client.Client, dbClient database.DatabaseClient, adbID *string, retryPolicy *common.RetryPolicy) (database.AutonomousDatabaseLifecycleStateEnum, error) {
	resp, err := e2eutil.GetAutonomousDatabase(ctx, dbClient, adbID, retryPolicy)
	if err != nil {
		return "", err
	}
...
func returnACDLocalState(ctx context.Context, k8sClient client.Client, acdLookupKey types.NamespacedName) (database.AutonomousContainerDatabaseLifecycleStateEnum, error) {
	acd := &dbv1alpha1.AutonomousContainerDatabase{}
	err := k8sClient.Get(ctx, acdLookupKey, acd)
	if err != nil {
		return "", err
	}
...
func returnACDRemoteState(ctx context.Context, k8sClient client.Client, dbClient database.DatabaseClient, acdID *string, retryPolicy *common.RetryPolicy) (database.AutonomousContainerDatabaseLifecycleStateEnum, error) {
	resp, err := e2eutil.GetAutonomousContainerDatabase(ctx, dbClient, acdID, retryPolicy)
	if err != nil {
		return "", err
	}
	return resp.LifecycleState, nil
...
func ConfigureADBBackup(ctx context.Context, dbClient *database.DatabaseClient, databaseOCID *string, tnsEntry *string, adminPassword *string, walletPassword *string, bucket *string, authToken *string, ociUser *string) error {
...
func AssertBackupRestore(ctx context.Context, k8sClient *client.Client, dbClient *database.DatabaseClient, backupRestoreLookupKey *types.NamespacedName, adbLookupKey *types.NamespacedName, state database.AutonomousDatabaseLifecycleStateEnum) func() {
	return func() {
		...
		AssertADBRemoteStateForBackupRestore(ctx, k8sClient, dbClient, adbLookupKey, database.AutonomousDatabaseLifecycleStateAvailable)()
		...
		if state == database.AutonomousDatabaseLifecycleStateBackupInProgress {
			...
			Eventually(func() (database.AutonomousDatabaseBackupLifecycleStateEnum, error) {
				if err := derefK8sClient.Get(ctx, *backupRestoreLookupKey, createdBackup); err != nil {
					return "", err
				}
			...
			Eventually(func() (workrequests.WorkRequestStatusEnum, error) {
				if err := derefK8sClient.Get(ctx, *backupRestoreLookupKey, createdRestore); err != nil {
					return "", err
				}
...
func AssertACDState(ctx context.Context, k8sClient *client.Client, dbClient *database.DatabaseClient, acdLookupKey *types.NamespacedName, state database.AutonomousContainerDatabaseLifecycleStateEnum, timeout time.Duration) func() {
	return func() {
		AssertACDLocalState(ctx, k8sClient, acdLookupKey, state, timeout)()
		AssertACDRemoteState(ctx, k8sClient, dbClient, acdLookupKey, state, timeout)()
	}
}

func AssertACDLocalState(ctx context.Context, k8sClient *client.Client, acdLookupKey *types.NamespacedName, state database.AutonomousContainerDatabaseLifecycleStateEnum, timeout time.Duration) func() {
	return func() {
		Expect(k8sClient).NotTo(BeNil())
		...
		Eventually(func() (database.AutonomousContainerDatabaseLifecycleStateEnum, error) {
			return returnACDLocalState(ctx, derefK8sClient, *acdLookupKey)
		}, timeout).Should(Equal(state))
	}
}

func AssertACDRemoteState(ctx context.Context, k8sClient *client.Client, dbClient *database.DatabaseClient, acdLookupKey *types.NamespacedName, state database.AutonomousContainerDatabaseLifecycleStateEnum, timeout time.Duration) func() {
	return func() {
		derefK8sClient := *k8sClient

		acd := &dbv1alpha1.AutonomousContainerDatabase{}
		Expect(derefK8sClient.Get(ctx, *acdLookupKey, acd)).To(Succeed())
		By("Checking if the lifecycleState of remote resource is " + string(state))
		AssertACDRemoteStateOCID(ctx, k8sClient, dbClient, acd.Spec.AutonomousContainerDatabaseOCID, state, timeout)()
	}
}

func AssertACDRemoteStateOCID(ctx context.Context, k8sClient *client.Client, dbClient *database.DatabaseClient, acdID *string, state database.AutonomousContainerDatabaseLifecycleStateEnum, timeout time.Duration) func() {
	return func() {
		Expect(k8sClient).NotTo(BeNil())
		Expect(dbClient).NotTo(BeNil())
		...
		Eventually(func() (database.AutonomousContainerDatabaseLifecycleStateEnum, error) {
			return returnACDRemoteState(ctx, derefK8sClient, derefDBClient, acdID, nil)
		}, timeout, intervalTime).Should(Equal(state))
	}
}

func AssertACDBind(ctx context.Context, k8sClient *client.Client, dbClient *database.DatabaseClient, acdLookupKey *types.NamespacedName, state database.AutonomousContainerDatabaseLifecycleStateEnum) func() {
	return func() {
		...
		AssertACDState(ctx, k8sClient, dbClient, acdLookupKey, state, acdBindTimeout)()
	}
}

func UpdateAndAssertACDSpec(ctx context.Context, k8sClient *client.Client, dbClient *database.DatabaseClient, acdLookupKey *types.NamespacedName) func() {
	return func() {
		expectedACD := UpdateACDSpec(ctx, k8sClient, dbClient, acdLookupKey)()
		AssertACDSpec(ctx, k8sClient, dbClient, acdLookupKey, expectedACD)()
	}
}

func UpdateACDSpec(ctx context.Context, k8sClient *client.Client, dbClient *database.DatabaseClient, acdLookupKey *types.NamespacedName) func() *dbv1alpha1.AutonomousContainerDatabase {
	return func() *dbv1alpha1.AutonomousContainerDatabase {
		...
		Expect(derefK8sClient.Get(ctx, *acdLookupKey, expectedAcd)).To(Succeed())

		expectedAcd.Spec.DisplayName = common.String(*expectedAcd.Spec.DisplayName + "_new")

		Expect(derefK8sClient.Update(ctx, expectedAcd)).To(Succeed())
		return expectedAcd
	}
}

func AssertACDSpec(ctx context.Context, k8sClient *client.Client, dbClient *database.DatabaseClient, acdLookupKey *types.NamespacedName, expectedACD *dbv1alpha1.AutonomousContainerDatabase) func() {
	return func() {
		Expect(k8sClient).NotTo(BeNil())
		Expect(dbClient).NotTo(BeNil())
		Expect(acdLookupKey).NotTo(BeNil())
		...
		Eventually(func() (bool, error) {
			...
			resp, err := e2eutil.GetAutonomousContainerDatabase(ctx, derefDBClient, expectedACD.Spec.AutonomousContainerDatabaseOCID, &retryPolicy)
			if err != nil {
				return false, err
			}
		...
		// IMPORTANT: make sure the local resource has finished reconciling, otherwise the changes will
		// be conflicted with the next test and cause unknow result.
		AssertACDLocalState(ctx, k8sClient, acdLookupKey, database.AutonomousContainerDatabaseLifecycleStateAvailable, time.Minute*2)()
	}
}

func AssertACDRestart(ctx context.Context, k8sClient *client.Client, dbClient *database.DatabaseClient, acdLookupKey *types.NamespacedName) func() {
	return func() {
		Expect(k8sClient).NotTo(BeNil())
		Expect(dbClient).NotTo(BeNil())
		Expect(acdLookupKey).NotTo(BeNil())
		...
		Expect(derefK8sClient.Update(ctx, acd))

		// Check ACD status is RESTARTING
		AssertACDState(ctx, k8sClient, dbClient, acdLookupKey, database.AutonomousContainerDatabaseLifecycleStateRestarting, time.Minute*2)()
		// Wait until restart is completed
		AssertACDState(ctx, k8sClient, dbClient, acdLookupKey, database.AutonomousContainerDatabaseLifecycleStateAvailable, time.Minute*7)()
	}
}

func AssertACDTerminate(ctx context.Context, k8sClient *client.Client, dbClient *database.DatabaseClient, acdLookupKey *types.NamespacedName) func() {
	return func() {
		Expect(k8sClient).NotTo(BeNil())
		Expect(dbClient).NotTo(BeNil())
		Expect(acdLookupKey).NotTo(BeNil())
		...
		derefK8sClient := *k8sClient
		acd := &dbv1alpha1.AutonomousContainerDatabase{}
		Expect(derefK8sClient.Get(ctx, *acdLookupKey, acd))

		acd.Spec.Action = dbv1alpha1.AcdActionTerminate
		Expect(derefK8sClient.Update(ctx, acd))

		// Check ACD status is TERMINATING
		AssertACDState(ctx, k8sClient, dbClient, acdLookupKey, database.AutonomousContainerDatabaseLifecycleStateTerminating, time.Minute*2)()
		// Wait until status is TERMINATED
		AssertACDState(ctx, k8sClient, dbClient, acdLookupKey, database.AutonomousContainerDatabaseLifecycleStateTerminated, time.Minute*40)()
	}
}

func AssertACDLocalDelete(ctx context.Context, k8sClient *client.Client, dbClient *database.DatabaseClient, acdLookupKey *types.NamespacedName) func() {
	return func() {
		Expect(k8sClient).NotTo(BeNil())
		Expect(dbClient).NotTo(BeNil())
		Expect(acdLookupKey).NotTo(BeNil())
		...
		Expect(derefK8sClient.Get(ctx, *acdLookupKey, existingAcd)).To(Succeed())
		Expect(derefK8sClient.Delete(ctx, existingAcd))

		By("Checking if the AutonomousContainerDatabase resource is deleted")
		Eventually(func() (isDeleted bool) {
			acd := &dbv1alpha1.AutonomousContainerDatabase{}
			isDeleted = false
			err := derefK8sClient.Get(ctx, *acdLookupKey, acd)
			if err != nil && k8sErrors.IsNotFound(err) {
				isDeleted = true
				return
			}
		...
		AssertACDRemoteState(ctx, k8sClient, dbClient, acdLookupKey, database.AutonomousContainerDatabaseLifecycleStateAvailable, time.Minute*2)
	}
}
