...
var _ = Describe("test ADB provisioning", func() {
	...
	Describe("Using adminPasswordSecret and hardLink=true", func() {
		...
		It("Should create a AutonomousDatabase resource", func() {
			...
			Expect(k8sClient.Create(e2eCtx, adb)).To(Succeed())
		})

		It("Should provision ADB using the admin password from K8s Secret "+SharedAdminPassSecretName, e2ebehavior.AssertProvision(e2eCtx, &k8sClient, &adbLookupKey))
		...
		It("Should try to provision ADB with duplicate db name", func() {
			...
			Expect(k8sClient.Create(e2eCtx, duplicateAdb)).To(Succeed())
		})

		It("Should check for local resource state \"\"", e2ebehavior.AssertADBLocalState(e2eCtx, &k8sClient, &dupAdbLookupKey, ""))
		...
		It("Should cleanup the resource with duplicated db name", func() {
			...
			Expect(k8sClient.Delete(e2eCtx, duplicateAdb)).To(Succeed())
		})

		It("Should create an Autonomous Database Backup", func() {
			e2ebehavior.AssertADBState(e2eCtx, &k8sClient, &dbClient, &adbLookupKey, database.AutonomousDatabaseLifecycleStateAvailable)()

			// Get adb ocid
			adb := &dbv1alpha1.AutonomousDatabase{}
			Expect(k8sClient.Get(e2eCtx, adbLookupKey, adb)).To(Succeed())
			databaseOCID := adb.Spec.Details.Id
			tnsEntry := dbName + "_high"
			err := e2ebehavior.ConfigureADBBackup(e2eCtx, &dbClient, databaseOCID, &tnsEntry, &SharedPlainTextAdminPassword, &SharedPlainTextWalletPassword, &SharedBucketUrl, &SharedAuthToken, &SharedOciUser)
			...
			Expect(k8sClient.Create(e2eCtx, adbBackup)).To(Succeed())

			backupLookupKey := types.NamespacedName{Name: backupName, Namespace: ADBNamespace}
			e2ebehavior.AssertBackupRestore(e2eCtx, &k8sClient, &dbClient, &backupLookupKey, &adbLookupKey, database.AutonomousDatabaseLifecycleStateBackupInProgress)()
		})
		...
		It("Should restore a database", func() {
			...
			Expect(k8sClient.Create(e2eCtx, adbRestore)).To(Succeed())
			restoreLookupKey := types.NamespacedName{Name: restoreName, Namespace: ADBNamespace}
			e2ebehavior.AssertBackupRestore(e2eCtx, &k8sClient, &dbClient, &restoreLookupKey, &adbLookupKey, database.AutonomousDatabaseLifecycleStateRestoreInProgress)()
		})
	...
	Describe("Using adminPasswordOCID and hardLink=true", func() {
		...
		It("Should provision ADB using the password from OCI Secret OCID "+SharedAdminPasswordOCID, e2ebehavior.AssertProvision(e2eCtx, &k8sClient, &adbLookupKey))
