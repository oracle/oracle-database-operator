...
func buildContainerSpecForShard(instance *databasev4.ShardingDatabase, OraShardSpex databasev4.ShardSpec) []corev1.Container {
	...
	containerSpec := corev1.Container{
		...
		// StartupProbe: buildShellExecProbe(dbCheckCmd, 30, 180, 30, 60),
...
func volumeClaimTemplatesForShard(instance *databasev4.ShardingDatabase, OraShardSpex databasev4.ShardSpec) []corev1.PersistentVolumeClaim {
	...
	for _, pvcMount := range pvcMounts {
		...
		if pvcMount.mountPath == databasev4.DefaultOraDataMountPath {
			if annotations := storageSourceAnnotations(OraShardSpex.StorageSource); len(annotations) > 0 {
...
func storageSourceAnnotations(source *databasev4.StorageSource) map[string]string {
	if source == nil || source.OCI == nil {
		return nil
	}
	backupID := strings.TrimSpace(source.OCI.VolumeBackupID)
	if backupID == "" {
		return nil
	}
	return map[string]string{
