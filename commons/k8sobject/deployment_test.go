...
func TestReconcileDeploymentRetriesConflict(t *testing.T) {
	sch := runtime.NewScheme()
	if err := appsv1.AddToScheme(sch); err != nil {
		t.Fatalf("failed to add appsv1 scheme: %v", err)
	}
	...
	existing := &appsv1.Deployment{
		ObjectMeta: metav1.ObjectMeta{Name: "dep-c", Namespace: "ns-c"},
		Spec: appsv1.DeploymentSpec{
			Replicas: ptr.To(int32(1)),
			Selector: &metav1.LabelSelector{MatchLabels: map[string]string{"app": "dep-c"}},
			Template: corev1.PodTemplateSpec{
				ObjectMeta: metav1.ObjectMeta{Labels: map[string]string{"app": "dep-c"}},
				Spec: corev1.PodSpec{
					Containers: []corev1.Container{{Name: "c", Image: "busybox:1"}},
				},
			},
	...
	baseClient := fake.NewClientBuilder().WithScheme(sch).WithObjects(existing).Build()
	cl := &conflictOnceDeploymentClient{Client: baseClient}

	var updateCalls int
	_, result, err := ReconcileDeployment(context.Background(), cl, "ns-c", desired, func(found, desired *appsv1.Deployment) bool {
		updateCalls++
		found.Spec.Template.Spec.Containers[0].Image = desired.Spec.Template.Spec.Containers[0].Image
		return true
	})
	...
	if updateCalls != 2 {
		t.Fatalf("expected update callback to run twice after one conflict, got %d", updateCalls)
	}

	updated := &appsv1.Deployment{}
	if err := baseClient.Get(context.Background(), types.NamespacedName{Name: "dep-c", Namespace: "ns-c"}, updated); err != nil {
		t.Fatalf("failed to fetch updated deployment: %v", err)
	}
	if got := updated.Spec.Template.Spec.Containers[0].Image; got != "busybox:2" {
		t.Fatalf("expected image busybox:2, got %s", got)
...
type conflictOnceDeploymentClient struct {
	client.Client
	conflicted bool
}

func (c *conflictOnceDeploymentClient) Update(ctx context.Context, obj client.Object, opts ...client.UpdateOption) error {
	if _, ok := obj.(*appsv1.Deployment); ok && !c.conflicted {
		c.conflicted = true
