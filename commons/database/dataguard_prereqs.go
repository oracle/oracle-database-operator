...
func BuildDataguardPrereqsCommand(action, brokerConfigDir, standbyRedoSize string) string {
	...
	lines = append(lines, `export SCRIPT_BASE_DIR="${SCRIPT_BASE_DIR:-/opt/oracle/scripts/base}"`)
	...
	lines = append(lines, fmt.Sprintf(`if [ -x "$SCRIPT_BASE_DIR/${CONFIG_DG_PREREQS_FILE:-configDataguardPrereqs.sh}" ]; then "$SCRIPT_BASE_DIR/${CONFIG_DG_PREREQS_FILE:-configDataguardPrereqs.sh}" %s; else "$ORACLE_BASE/${CONFIG_DG_PREREQS_FILE:-configDataguardPrereqs.sh}" %s; fi`, trimmedAction, trimmedAction))
