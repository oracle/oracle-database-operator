...
func shouldPreserveAvailableClusterState(currentState, nextState string) bool {
	if currentState != string(racdb.RACAvailableState) {
		return false
	}

	switch nextState {
	case string(racdb.RACProvisionState),
		string(racdb.RACPendingState),
...
func UpdateRacNodeStatusDataForCluster(
	...
	if !shouldPreserveAvailableClusterState(racDatabase.Status.State, state) {
