...
func VolumePVCForASM(
	...
	if storageClassName := storageClassForAsmDiskGroup(instance, diskGroupName); storageClassName == "" {
		...
		// Prevent default StorageClass injection for local/raw ASM PVCs on clusters
		// that define a default class (for example rook-ceph on OCNE).
		noStorageClass := ""
		asmPvc.Spec.StorageClassName = &noStorageClass
	} else {
...
func DelRacSwPvcClusterStyle(instance *racdb.RacDatabase, clusterSpec *racdb.RacClusterDetailSpec, nodeIndex int, kClient client.Client, logger logr.Logger) error {
	...
	if instance.Spec.IsKeep {
		LogMessages("DEBUG", "Skipping software PVC deletion because spec.isKeep=true", nil, instance, logger)
		return nil
	}
	...
	if swMode != racdb.RacSwStorageStorageClass {
		return nil
	}
	pvcName := fmt.Sprintf("%s-%s-0", GetClusterSwPvcName(nodeName), nodeName)
...
func deletePVCIfExists(instance *racdb.RacDatabase, pvcName string, kClient client.Client, logger logr.Logger) error {
	...
	if err != nil {
		...
		LogMessages("DEBUG", "Error occurred in finding the pvc claim!", err, instance, logger)
		return err
	}
	if len(pvcFound.GetFinalizers()) > 0 {
		LogMessages("DEBUG", "Removing PVC finalizers", nil, instance, logger)
		pvcFound.SetFinalizers([]string{})
		if err := kClient.Update(context.Background(), pvcFound); err != nil {
			LogMessages("DEBUG", "Error occurred in removing pvc finalizers!", err, instance, logger)
			return err
		}
	}
	if err := kClient.Delete(context.Background(), pvcFound); err != nil && !apierrors.IsNotFound(err) {
		LogMessages("DEBUG", "Error occurred in deleting the pvc claim!", err, instance, logger)
		return err
	}
	return nil
}

// DelRacPvc removes the shared ASM PVC associated with the specified disk.
func DelRacPvc(instance *racdb.RacDatabase, diskName string, kClient client.Client, logger logr.Logger) error {
...
func GetDbSecret(instance *racdb.RacDatabase, name string, kClient client.Client) (bool, bool, bool) {
	...
	commonpassflag, pwdkeyflag, legacyPwdFile := sharedinstanceutil.DBSecretFlags(
		secretFound.Data,
		instance.Spec.DbSecret.PwdFileName,
		keyFileName,
	)
	if !pwdkeyflag && instance.Spec.DbSecret.KeySecretName != "" {
		keySecret, err := CheckSecret(instance, instance.Spec.DbSecret.KeySecretName, kClient)
		if err == nil {
			_, pwdkeyflag, _ = sharedinstanceutil.DBSecretFlags(keySecret.Data, "", keyFileName)
		}
	}
	return commonpassflag, pwdkeyflag, legacyPwdFile
}
...
func GetTdeWalletSecret(instance *racdb.RacDatabase, name string, kClient client.Client) (bool, bool, bool) {
	...
	if !pwdkeyflag && instance.Spec.TdeWalletSecret.KeySecretName != "" {
		keySecret, err := CheckSecret(instance, instance.Spec.TdeWalletSecret.KeySecretName, kClient)
		if err == nil {
			for key := range keySecret.Data {
				if key == keyFileName {
					pwdkeyflag = true
				}
			}
