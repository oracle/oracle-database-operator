...
func buildVolumeMountSpecForRacCluster(
	...
	if swModeErr == nil && swMode != racdb.RacSwStorageNone {
		...
		swVolumeName := nodeName + "-oradata-sw-vol"
		if swMode == racdb.RacSwStorageStorageClass {
			// StatefulSet volumeClaimTemplates inject a pod volume using the claim template name.
			swVolumeName = GetClusterSwPvcName(nodeName)
		}
		result = append(result, corev1.VolumeMount{
			Name:      swVolumeName,
