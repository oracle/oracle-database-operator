...
func setDataguardBrokerRunnerOwner(t *testing.T, scheme *runtime.Scheme, broker *dbapi.DataguardBroker, pod *corev1.Pod) {
	t.Helper()
	if err := ctrl.SetControllerReference(broker, pod, scheme); err != nil {
		t.Fatalf("failed to set runner pod owner: %v", err)
	}
}

func TestParseDataguardTopologyFSFOStatusNormalizesDGMGRLSpacing(t *testing.T) {
	status, ok := parseDataguardTopologyFSFOStatus(`
Fast-Start Failover:  Disabled
  Protection Mode:    MaxAvailability
`)
	if !ok || status != "false" {
		t.Fatalf("expected disabled FSFO status, got status=%q ok=%t", status, ok)
	}

	status, ok = parseDataguardTopologyFSFOStatus("Fast-Start Failover:\tEnabled\n")
	if !ok || status != "true" {
		t.Fatalf("expected enabled FSFO status, got status=%q ok=%t", status, ok)
	}
}

func TestDeleteDataguardTopologyObserverPodDeletesOnlyNamedObserver(t *testing.T) {
	...
	reconciler := &DataguardBrokerReconciler{
		Client: fake.NewClientBuilder().WithScheme(scheme).WithObjects(observerPod, otherPod).Build(),
		Scheme: scheme,
	}

	if err := deleteDataguardTopologyObserverPod(context.Background(), reconciler, broker); err != nil {
		t.Fatalf("expected observer pod delete to succeed, got %v", err)
	}

	var deletedObserver corev1.Pod
	err := reconciler.Get(context.Background(), types.NamespacedName{Name: "dg-prod-observer", Namespace: "default"}, &deletedObserver)
	if !apierrors.IsNotFound(err) {
		t.Fatalf("expected observer pod to be deleted, got err=%v", err)
	}

	var remainingPod corev1.Pod
	if err := reconciler.Get(context.Background(), types.NamespacedName{Name: "dg-prod-runner", Namespace: "default"}, &remainingPod); err != nil {
		t.Fatalf("expected non-observer pod to remain, got %v", err)
	}
}

func TestDataguardTopologyLocalMemberReadyUsesSIDBHealth(t *testing.T) {
	scheme := runtime.NewScheme()
	if err := dbapi.AddToScheme(scheme); err != nil {
		t.Fatalf("failed to add database scheme: %v", err)
	}
	...
	sidb := &dbapi.SingleInstanceDatabase{
		...
		Status: dbapi.SingleInstanceDatabaseStatus{Status: dbcommons.StatusNotReady},
	}
	reconciler := &DataguardBrokerReconciler{
	...
	ready, message, err := dataguardTopologyLocalMemberReady(context.Background(), reconciler, broker, member)
	if err != nil {
		t.Fatalf("expected local member health check to succeed, got %v", err)
	}
	if ready {
		t.Fatalf("expected unhealthy SIDB member to block broker readiness")
	}
	if !strings.Contains(message, "waiting for database recovery") {
		t.Fatalf("expected recovery wait message, got %q", message)
	}

	sidb.Status.Status = dbcommons.StatusReady
	if err := reconciler.Update(context.Background(), sidb); err != nil {
		t.Fatalf("failed to update sidb status: %v", err)
	}
	ready, message, err = dataguardTopologyLocalMemberReady(context.Background(), reconciler, broker, member)
	if err != nil {
		t.Fatalf("expected local member health check to succeed, got %v", err)
	...
	if !ready || message != "" {
		t.Fatalf("expected healthy SIDB member to pass, ready=%t message=%q", ready, message)
	}
}
...
func TestEnsureDataguardBrokerRunnerPodDeletesStaleRunnerAfterDesiredReady(t *testing.T) {
	...
	setDataguardBrokerRunnerOwner(t, scheme, broker, desiredPod)
	desiredPod.Status.Phase = corev1.PodRunning

	stalePod := buildDataguardBrokerRunnerPod(broker, runtimeSpec, "stalehash00")
	setDataguardBrokerRunnerOwner(t, scheme, broker, stalePod)
	stalePod.Status.Phase = corev1.PodRunning
...
func TestDataguardBrokerRunnerPodRejectsForeignSameNamePod(t *testing.T) {
	scheme := runtime.NewScheme()
	if err := dbapi.AddToScheme(scheme); err != nil {
		t.Fatalf("failed to add dbapi scheme: %v", err)
	}
	...
	broker := &dbapi.DataguardBroker{
		ObjectMeta: metav1.ObjectMeta{Name: "dg", Namespace: "ns1", UID: types.UID("broker-uid")},
		Spec: dbapi.DataguardBrokerSpec{
			Execution: &dbapi.DataguardExecutionSpec{
				Image:            "runner:19c",
				ImagePullSecrets: []string{"pull-secret"},
		...
		Status: dbapi.DataguardBrokerStatus{
			ObservedTopologyHash: "topology-hash-foreign",
		},
	}
	runtimeSpec := &dataguardBrokerExecutionRuntime{
		Image:               "runner:19c",
	...
	runtimeHash := computeDataguardBrokerRunnerRuntimeHash(broker, runtimeSpec)
	podName := dataguardBrokerRunnerPodNameForHash(broker, runtimeHash)
	foreignPod := &corev1.Pod{
		ObjectMeta: metav1.ObjectMeta{Name: podName, Namespace: broker.Namespace},
		Spec: corev1.PodSpec{
			Containers: []corev1.Container{{Name: dataguardBrokerRunnerContainerName}},
		},
		Status: corev1.PodStatus{Phase: corev1.PodRunning},
	}

	reconciler := &DataguardBrokerReconciler{
		Client: fake.NewClientBuilder().WithScheme(scheme).WithObjects(foreignPod).Build(),
		Scheme: scheme,
	}
	...
	if ready {
		t.Fatalf("expected foreign same-name runner pod to be rejected")
	}
	if message == "" {
		t.Fatalf("expected rejection message")
	}
	var stillThere corev1.Pod
	if err := reconciler.Get(context.Background(), types.NamespacedName{Name: podName, Namespace: broker.Namespace}, &stillThere); err != nil {
		t.Fatalf("expected foreign same-name pod to be preserved: %v", err)
	}
	if _, err := resolveDataguardBrokerActiveRunnerPod(context.Background(), reconciler, broker); err == nil {
		t.Fatalf("expected active runner resolver to reject foreign same-name pod")
	} else if !isDataguardBrokerRunnerUnavailable(err) {
		t.Fatalf("expected runner unavailable error, got %v", err)
	}
}

func TestDataguardBrokerRunnerPodRejectsMissingRunnerContainer(t *testing.T) {
	scheme := runtime.NewScheme()
	if err := dbapi.AddToScheme(scheme); err != nil {
		t.Fatalf("failed to add dbapi scheme: %v", err)
	}
	...
	broker := &dbapi.DataguardBroker{
		...
		Status: dbapi.DataguardBrokerStatus{
			ObservedTopologyHash: "topology-hash-container",
		},
	}
	runtimeSpec := &dataguardBrokerExecutionRuntime{
		Image:               "runner:19c",
	...
	runtimeHash := computeDataguardBrokerRunnerRuntimeHash(broker, runtimeSpec)
	pod := buildDataguardBrokerRunnerPod(broker, runtimeSpec, runtimeHash)
	setDataguardBrokerRunnerOwner(t, scheme, broker, pod)
	pod.Spec.Containers[0].Name = "not-runner"
	...
	if _, err := resolveDataguardBrokerActiveRunnerPod(context.Background(), reconciler, broker); err == nil {
		t.Fatalf("expected active runner resolver to reject pod without runner container")
	} else if !isDataguardBrokerRunnerUnavailable(err) {
		t.Fatalf("expected runner unavailable error, got %v", err)
	}
	...
	if ready {
		t.Fatalf("expected invalid managed runner pod to be recreated")
	}
	if message == "" {
		t.Fatalf("expected recreate message")
	}
	var deletedCheck corev1.Pod
	if err := reconciler.Get(context.Background(), types.NamespacedName{Name: pod.Name, Namespace: broker.Namespace}, &deletedCheck); err == nil {
		t.Fatalf("expected invalid managed runner pod %q to be deleted", pod.Name)
	}
}
...
func TestResolveDataguardTopologyMemberAdminSecretRefRejectsCrossNamespaceLocalRef(t *testing.T) {
	scheme := runtime.NewScheme()
	if err := dbapi.AddToScheme(scheme); err != nil {
		t.Fatalf("failed to add dbapi scheme: %v", err)
	}
	...
	sidb := &dbapi.SingleInstanceDatabase{}
	sidb.Namespace = "other-ns"
	sidb.Name = "sidb-standby"
	sidb.Spec.Security = &dbapi.SingleInstanceDatabaseSecurity{
		Secrets: &dbapi.SingleInstanceDatabaseSecrets{
			Admin: &dbapi.SingleInstanceDatabaseAdminPassword{
				SecretName: "foreign-admin",
				SecretKey:  "oracle_pwd",
			},
		},
	}
	...
	_, _, _, err := resolveDataguardTopologyMemberAdminSecretRef(context.Background(), reconciler, broker, member)
	if err == nil {
		t.Fatalf("expected cross-namespace localRef to be rejected")
	}
	if !strings.Contains(err.Error(), "must match DataguardBroker namespace") {
		t.Fatalf("expected localRef namespace validation error, got: %v", err)
	}
}
