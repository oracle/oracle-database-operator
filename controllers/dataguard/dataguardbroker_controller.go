...
func (r *DataguardBrokerReconciler) reconcileDataguardBrokerNormal(ctx context.Context, scope *dataguardBrokerReconcileScope) (ctrl.Result, error) {
	...
	if desired.Path == dataguardBrokerPathTopology {
		scope.setTopologyResolvedCondition(true, "TopologyResolved", "resolved topology members and pairs")
	}
...
func (r *DataguardBrokerReconciler) reconcileDataguardBrokerManualSwitchover(ctx context.Context, scope *dataguardBrokerReconcileScope) (ctrl.Result, error) {
	...
	if _, ok := scope.broker.Status.DatabasesInDataguardConfig[targetSidbSid]; !ok {
		r.Recorder.Eventf(scope.broker, corev1.EventTypeWarning, "Cannot Switchover", fmt.Sprintf("database with SID %v not found in dataguardbroker configuration", targetSidbSid))
	...
	r.Recorder.Eventf(scope.broker, corev1.EventTypeWarning, "Manual Switchover", fmt.Sprintf("Switching over to %s database", scope.broker.Status.DatabasesInDataguardConfig[targetSidbSid]))
