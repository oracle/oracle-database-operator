...
const (
	dataguardBrokerRunnerLabelBroker      = "database.oracle.com/dataguard-broker"
	dataguardBrokerRunnerLabelComponent   = "database.oracle.com/component"
	dataguardBrokerRunnerLabelRuntimeHash = "database.oracle.com/runtime-hash"
	dataguardBrokerRunnerAnnotationHash   = "database.oracle.com/runtime-hash"
	dataguardBrokerRunnerComponentValue   = "execution-runner"
	dataguardBrokerRunnerContainerName    = "runner"
)
...
func (s *dataguardBrokerReconcileScope) writeStatus(ctx context.Context, r *DataguardBrokerReconciler) error {
	if reflect.DeepEqual(s.originalStatus, s.broker.Status) {
		s.log.V(1).Info("Skipping DataguardBroker status update because status is unchanged",
			"status", strings.TrimSpace(s.broker.Status.Status))
		return nil
	}
	s.log.Info("Updating DataguardBroker status",
		"status", strings.TrimSpace(s.broker.Status.Status),
		"primary", strings.TrimSpace(s.broker.Status.PrimaryDatabase),
		"standbys", strings.TrimSpace(s.broker.Status.StandbyDatabases),
		"clusterConnectString", strings.TrimSpace(s.broker.Status.ClusterConnectString),
		"externalConnectString", strings.TrimSpace(s.broker.Status.ExternalConnectString))
	desiredStatus := *s.broker.Status.DeepCopy()
	if err := retry.RetryOnConflict(retry.DefaultRetry, func() error {
		var latest dbapi.DataguardBroker
		if err := r.Get(ctx, s.req.NamespacedName, &latest); err != nil {
			return err
		}
		...
		s.broker.ResourceVersion = latest.ResourceVersion
		return nil
	}); err != nil {
		if apierrors.IsConflict(err) {
			s.log.Info("DataguardBroker status update conflicted after retries; will retry on next reconcile",
				"status", strings.TrimSpace(s.broker.Status.Status))
		} else {
			s.log.Error(err, "Failed to update DataguardBroker status",
				"status", strings.TrimSpace(s.broker.Status.Status))
		}
		return err
	}
	s.log.Info("Updated DataguardBroker status successfully",
		"status", strings.TrimSpace(s.broker.Status.Status))
	return nil
}
...
func validateDataguardBrokerRunnerPodIdentity(pod *corev1.Pod, broker *dbapi.DataguardBroker, runtimeHash string) error {
	if pod == nil {
		return fmt.Errorf("runner pod is nil")
	}
	if broker == nil {
		return fmt.Errorf("broker is nil")
	}
	if !metav1.IsControlledBy(pod, broker) {
		return fmt.Errorf("runner pod %s/%s is not controlled by DataguardBroker %s/%s", pod.Namespace, pod.Name, broker.Namespace, broker.Name)
	}
	if got := strings.TrimSpace(pod.Labels[dataguardBrokerRunnerLabelBroker]); got != broker.Name {
		return fmt.Errorf("runner pod %s/%s has dataguard broker label %q, want %q", pod.Namespace, pod.Name, got, broker.Name)
	}
	if got := strings.TrimSpace(pod.Labels[dataguardBrokerRunnerLabelComponent]); got != dataguardBrokerRunnerComponentValue {
		return fmt.Errorf("runner pod %s/%s has component label %q, want %q", pod.Namespace, pod.Name, got, dataguardBrokerRunnerComponentValue)
	}
	expectedHash := strings.TrimSpace(runtimeHash)
	if got := strings.TrimSpace(pod.Annotations[dataguardBrokerRunnerAnnotationHash]); got != expectedHash {
		return fmt.Errorf("runner pod %s/%s has runtime hash annotation %q, want %q", pod.Namespace, pod.Name, got, expectedHash)
	}
	expectedHashLabel := dataguardBrokerRunnerRuntimeHashLabelValue(expectedHash)
	if got := strings.TrimSpace(pod.Labels[dataguardBrokerRunnerLabelRuntimeHash]); got != expectedHashLabel {
		return fmt.Errorf("runner pod %s/%s has runtime hash label %q, want %q", pod.Namespace, pod.Name, got, expectedHashLabel)
	}
	for i := range pod.Spec.Containers {
		if pod.Spec.Containers[i].Name == dataguardBrokerRunnerContainerName {
			return nil
		}
	}
	return fmt.Errorf("runner pod %s/%s does not contain container %q", pod.Namespace, pod.Name, dataguardBrokerRunnerContainerName)
}
...
func listDataguardBrokerRunnerPods(ctx context.Context, r client.Client, broker *dbapi.DataguardBroker) ([]corev1.Pod, error) {
	...
	if err := r.List(ctx, &pods,
		...
		client.MatchingLabels{
			dataguardBrokerRunnerLabelBroker:    broker.Name,
			dataguardBrokerRunnerLabelComponent: dataguardBrokerRunnerComponentValue,
		},
	); err != nil {
		return nil, err
	}
...
func ensureDataguardBrokerRunnerPod(ctx context.Context, r *DataguardBrokerReconciler, broker *dbapi.DataguardBroker, runtime *dataguardBrokerExecutionRuntime) (bool, string, error) {
	...
	if !existing.DeletionTimestamp.IsZero() {
		return false, fmt.Sprintf("topology execution runner pod %s is being deleted", podName), nil
	}
	if err := validateDataguardBrokerRunnerPodIdentity(&existing, broker, runtimeHash); err != nil {
		if !metav1.IsControlledBy(&existing, broker) {
			return false, fmt.Sprintf("topology execution runner pod %s is not managed by this DataguardBroker: %v", podName, err), nil
		}
		if deleteErr := clientIgnoreNotFound(r.Delete(ctx, &existing)); deleteErr != nil {
			return false, "", deleteErr
		}
		return false, fmt.Sprintf("topology execution runner pod %s has invalid identity; deleting for recreation: %v", podName, err), nil
	}
...
func buildDataguardBrokerRunnerPod(broker *dbapi.DataguardBroker, runtime *dataguardBrokerExecutionRuntime, runtimeHash string) *corev1.Pod {
	...
	labels := map[string]string{
		dataguardBrokerRunnerLabelBroker:      broker.Name,
		dataguardBrokerRunnerLabelComponent:   dataguardBrokerRunnerComponentValue,
		dataguardBrokerRunnerLabelRuntimeHash: dataguardBrokerRunnerRuntimeHashLabelValue(runtimeHash),
	}
	annotations := map[string]string{
		"database.oracle.com/runtime-image": strings.TrimSpace(runtime.Image),
		dataguardBrokerRunnerAnnotationHash: strings.TrimSpace(runtimeHash),
	}
	...
	return &corev1.Pod{
		...
		Spec: corev1.PodSpec{
			...
			Containers: []corev1.Container{{
				Name:            dataguardBrokerRunnerContainerName,
				Image:           runtime.Image,
