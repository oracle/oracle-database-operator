...
func (r *LRPDBReconciler) OpenLRPDB(ctx context.Context, req ctrl.Request, lrpdb *dbapi.LRPDB) error {
	...
	if lrpdb.Spec.LRPDBState == "OPEN" {
		if Bit(lrpdb.Status.CmBitstat, MPWARN|MPINIT) {
			...
			if _, err := r.ApplyConfigMap(ctx, req, lrpdb); err != nil {
				return err
			}
...
func (r *LRPDBReconciler) getLRPDBState(ctx context.Context, req ctrl.Request, lrpdb *dbapi.LRPDB) error {
	...
	if err := r.GetOpenMode(respData, &(lrpdb.Status.OpenMode)); err != nil {
		return err
	}
	if err := r.GetRestricted(respData, &(lrpdb.Status.Restricted)); err != nil {
		return err
	}
	if err := r.GetPdbSize2(respData, &(lrpdb.Status.TotalSize)); err != nil {
		return err
	}
