...
const (
	...
	initContainerName             = "ordssrvs-init"
	mainContainerName             = "ordssrvs-main"
	accessLogContainerName        = "ordssrvs-access-log-forwarder"
	...
	APEXInstallationLabel         = "apex-inst"
	APEXInstallationMount         = "/opt/oracle/apex"
	AccessLogLabel                = "access-log"
	AccessLogMount                = ordsSABase + "/log/global/"
	APEXInstallationPodVolumeName = APEXInstallationLabel + "-vol"
	AccessLogPodVolumeName        = AccessLogLabel + "-vol"
)
...
type OrdsSrvsReconcileState struct {
	...
	APEXInstallationExternal            bool
	APEXInstallationPVCName             string
	AccessLogExternal                   bool
	AccessLogPVCName                    string
...
func (r *OrdsSrvsReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	// Treat standalone.https.port: 0 as HTTP-only mode for generated Kubernetes resources.
	rState.httpsEnabled = true
	if ordssrvs.Spec.GlobalSettings.StandaloneHTTPSPort != nil &&
		*ordssrvs.Spec.GlobalSettings.StandaloneHTTPSPort == 0 {
		rState.specDebug.Info("Disabling HTTPS because standalone.https.port is 0")
		rState.httpsEnabled = false
	}
	...
	rState.APEXInstallationExternal = persistenceRequested(ordssrvs.Spec.GlobalSettings.APEXInstallationPersistence)
	rState.AccessLogExternal = persistenceRequested(ordssrvs.Spec.AccessLogPersistence)

	if stop, err := r.validateSpec(ctx, req, ordssrvs, rState); stop || err != nil {
		return ctrl.Result{}, err
	}

	// APEXInstallationExternal
	if rState.APEXInstallationExternal {
		rState.specDebug.Info("Configuring external APEX installation files")
		rState.APEXInstallationPVCName = ordssrvs.Name + "-" + APEXInstallationLabel + "-pvc"
		if err := r.pvcReconcile(ctx, ordssrvs, rState, rState.APEXInstallationPVCName, APEXInstallationPodVolumeName, ordssrvs.Spec.GlobalSettings.APEXInstallationPersistence); err != nil {
	...
	// AccessLogExternal
	if rState.AccessLogExternal {
		rState.specDebug.Info("Configuring external access log")
		rState.AccessLogPVCName = ordssrvs.Name + "-" + AccessLogLabel + "-pvc"
		if err := r.pvcReconcile(ctx, ordssrvs, rState, rState.AccessLogPVCName, AccessLogPodVolumeName, ordssrvs.Spec.AccessLogPersistence); err != nil {
			logger.Error(err, "Error in AccessLog PVC reconcile")
			return ctrl.Result{}, err
		}
	}
...
func (r *OrdsSrvsReconciler) pvcReconcile(ctx context.Context, ordssrvs *dbapi.OrdsSrvs, rState *OrdsSrvsReconcileState, pvcName string, podVolumeName string, persistence dbapi.OrdsSrvsPersistence) (err error) {

	logger := log.FromContext(ctx).WithName("PVCReconcile")

	pvc := &corev1.PersistentVolumeClaim{}
	err = r.Get(ctx, types.NamespacedName{Name: pvcName, Namespace: ordssrvs.Namespace}, pvc)
	if err == nil {
		if err := validatePVCConsistency(ordssrvs, pvcName, pvc, persistence); err != nil {
			r.Recorder.Eventf(ordssrvs, corev1.EventTypeWarning, "PVCConflict", "%s", err.Error())
			return err
		}
		rState.specDebug.Info("Found pvc : " + pvcName)
		return nil
	}
	if !apierrors.IsNotFound(err) {
		return err
	...
	pvc, err = r.pvcDefine(ordssrvs, rState, pvcName, persistence)
	if err != nil {
		logger.Error(err, "Invalid "+pvcName+" definition")
		return err
	}

	message := fmt.Sprintf("PVC : %s for Pod Volume : %s", pvcName, podVolumeName)
...
func (r *OrdsSrvsReconciler) podTemplateSpecDefine(
	...
	imagePullPolicy := ordssrvs.Spec.ImagePullPolicy
	if imagePullPolicy == "" {
	...
	if ordssrvs.Spec.AccessLogForwarder.Enabled {
		podSpecTemplate.Spec.Containers = append(
			podSpecTemplate.Spec.Containers,
			accessLogForwarderContainerDefine(ordssrvs, rState, imagePullPolicy),
		)
	}
...
func accessLogForwarderContainerDefine(ordssrvs *dbapi.OrdsSrvs, rState *OrdsSrvsReconcileState, imagePullPolicy corev1.PullPolicy) corev1.Container {
	var resources corev1.ResourceRequirements
	if ordssrvs.Spec.AccessLogForwarder.Resources != nil {
		resources = *ordssrvs.Spec.AccessLogForwarder.Resources
	}

	return corev1.Container{
		Image:           ordssrvs.Spec.Image,
		Name:            accessLogContainerName,
		ImagePullPolicy: imagePullPolicy,
		Resources:       resources,
		SecurityContext: securityContextDefine(),
		Command:         []string{"/bin/bash", "-c", ordsSABase + "/scripts/access_log_forwarder.sh"},
		Env:             podIdentityEnvVarDefines(),
		VolumeMounts:    accessLogForwarderVolumeMountsDefine(rState),
	}
}

// podNameEnvVarDefine defines the PODNAME env variable from the Pod metadata.name field.
func podNameEnvVarDefine() corev1.EnvVar {
...
// podUIDEnvVarDefine defines the PODUID env variable from the Pod metadata.uid field.
func podUIDEnvVarDefine() corev1.EnvVar {
	return corev1.EnvVar{
		Name: "PODUID",
		ValueFrom: &corev1.EnvVarSource{
			FieldRef: &corev1.ObjectFieldSelector{
...
// podIdentityEnvVarDefines defines the Pod identity env vars used by scripts and subPathExpr.
func podIdentityEnvVarDefines() []corev1.EnvVar {
	return []corev1.EnvVar{
		podNameEnvVarDefine(),
		podUIDEnvVarDefine(),
	}
...
// accessLogSubPathExprDefine defines the PVC subpath for a single Pod object.
func accessLogSubPathExprDefine() string {
	return "$(PODNAME)-$(PODUID)"
}

// accessLogForwarderVolumeMountsDefine mounts only the script and access-log directories in the forwarder sidecar.
func accessLogForwarderVolumeMountsDefine(rState *OrdsSrvsReconcileState) []corev1.VolumeMount {
	accessLogVolumeName := AccessLogLabel
	if rState.AccessLogExternal {
		accessLogVolumeName = AccessLogPodVolumeName
	}

	accessLogVolumeMount := volumeMountBuild(accessLogVolumeName, AccessLogMount, true)
	if rState.AccessLogExternal {
		accessLogVolumeMount.SubPathExpr = accessLogSubPathExprDefine()
	}

	return []corev1.VolumeMount{
		volumeMountBuild(rState.ordssrvsScriptsConfigMapName, ordsSABase+"/scripts", true),
		accessLogVolumeMount,
	}
}
...
func (r *OrdsSrvsReconciler) VolumesDefine(ordssrvs *dbapi.OrdsSrvs, rState *OrdsSrvsReconcileState) ([]corev1.Volume, []corev1.VolumeMount) {
	...
	if rState.APEXInstallationExternal {
		pvcName := rState.APEXInstallationPVCName
		apexInstallationVolume := pvcVolumeDefine(APEXInstallationPodVolumeName, pvcName)
		apexInstallationVolumeMount := volumeMountBuild(APEXInstallationPodVolumeName, APEXInstallationMount, false)
	...
	// AccessLog
	var accessLogVolume corev1.Volume
	if rState.AccessLogExternal {
		accessLogVolume = pvcVolumeDefine(AccessLogPodVolumeName, rState.AccessLogPVCName)
	} else {
		accessLogVolume = emptyDirVolumeBuild(AccessLogLabel)
	}
	accessLogVolumeMount := volumeMountBuild(accessLogVolume.Name, AccessLogMount, false)
	if rState.AccessLogExternal {
		// PODUID gives each recreated Pod object a new access-log directory on shared PVCs.
		accessLogVolumeMount.SubPathExpr = accessLogSubPathExprDefine()
	}
	...
	volumes = append(volumes, globalVolume, accessLogVolume, globalConfigVolume)
	volumeMounts = append(volumeMounts, globalVolumeMount, accessLogVolumeMount, globalConfigVolumeMount)
...
func (r *OrdsSrvsReconciler) addSecretEnvVar(ctx context.Context, envVars []corev1.EnvVar, ordssrvs *dbapi.OrdsSrvs, envName string, secretName string, secretKey string) ([]corev1.EnvVar, error) {
	...
	if err != nil {
		if apierrors.IsNotFound(err) {
			e := fmt.Errorf("required secret not found")
			r.Recorder.Eventf(ordssrvs, corev1.EventTypeWarning, "SecretNotFound", "Failed to resolve env %q", envName)
			logger.Error(e, "SecretNotFound", "env", envName)
			return envVars, e
		}
		e := fmt.Errorf("secret read error")
	...
	if _, exists := secret.Data[secretKey]; !exists {
		e := fmt.Errorf("missing secret key")
...
func (r *OrdsSrvsReconciler) envDefine(ctx context.Context, ordssrvs *dbapi.OrdsSrvs, rState *OrdsSrvsReconcileState, initContainer bool) ([]corev1.EnvVar, error) {
	...
	// PODNAME and PODUID are available to scripts and volume SubPathExpr expansions.
	envVars = append(envVars, podIdentityEnvVarDefines()...)
	...
	if initContainer {
		...
		if instanceAPIAdminUser != "" && instanceAPIAdminSecretName != "" {
			...
			envVars, err = r.addSecretEnvVar(ctx, envVars, ordssrvs, envName, instanceAPIAdminSecretName, passwordKey)
			if err != nil {
		...
		for i := 0; i < len(ordssrvs.Spec.PoolSettings); i++ {
			...
			if ordssrvs.Spec.PoolSettings[i].DBUsername != "" {
				...
				if ordssrvs.Spec.PoolSettings[i].DBSecret.SecretName != "" {
					...
					var err error
					envVars, err = r.addSecretEnvVar(ctx, envVars, ordssrvs, envName, secretName, secretKey)
					if err != nil {
						return envVars, fmt.Errorf("resolve env %q: %w", envName, err)
					}
				}
...
func validatePVCConsistency(ordssrvs *dbapi.OrdsSrvs, pvcName string, pvc *corev1.PersistentVolumeClaim, persistence dbapi.OrdsSrvsPersistence) error {
	if !metav1.IsControlledBy(pvc, ordssrvs) {
		return fmt.Errorf("pvc %s/%s is not controlled by OrdsSrvs %s/%s", ordssrvs.Namespace, pvcName, ordssrvs.Namespace, ordssrvs.Name)
	}
	...
	expectedStorageClassName := persistence.StorageClass
	if pvc.Spec.StorageClassName == nil || *pvc.Spec.StorageClassName != expectedStorageClassName {
		return fmt.Errorf("pvc %s/%s storageClassName mismatch: expected %q, found %s", ordssrvs.Namespace, pvcName, expectedStorageClassName, storageClassNameForMessage(pvc.Spec.StorageClassName))
	}

	expectedAccessMode := persistence.AccessMode
	if expectedAccessMode == "" {
		expectedAccessMode = string(corev1.ReadWriteOnce)
	}
	expectedAccessModes := []corev1.PersistentVolumeAccessMode{corev1.PersistentVolumeAccessMode(expectedAccessMode)}
	if !reflect.DeepEqual(pvc.Spec.AccessModes, expectedAccessModes) {
		return fmt.Errorf("pvc %s/%s accessModes mismatch: expected %v, found %v", ordssrvs.Namespace, pvcName, expectedAccessModes, pvc.Spec.AccessModes)
	}

	return nil
}

func storageClassNameForMessage(storageClassName *string) string {
	if storageClassName == nil {
		return "<nil>"
	}
	return fmt.Sprintf("%q", *storageClassName)
}

// pvcVolumeDefine defines a Pod volume backed by a PVC.
func pvcVolumeDefine(volumeName string, pvcName string) corev1.Volume {
	return corev1.Volume{
		Name: volumeName,
		VolumeSource: corev1.VolumeSource{
...
// persistenceVolumeClaimDefine defines a PVC from storage attributes.
func (r *OrdsSrvsReconciler) pvcDefine(ordssrvs *dbapi.OrdsSrvs, rState *OrdsSrvsReconcileState, pvcName string, persistence dbapi.OrdsSrvsPersistence) (*corev1.PersistentVolumeClaim, error) {

	size := persistence.Size
	pvName := persistence.VolumeName
	storageClassName := persistence.StorageClass
	accessMode := persistence.AccessMode
	...
	if err := ctrl.SetControllerReference(ordssrvs, pvc, r.Scheme); err != nil {
		return nil, fmt.Errorf("set owner reference for PVC %s/%s: %w", ordssrvs.Namespace, pvcName, err)
	}
...
// validateSpec checks OrdsSrvs combinations that cannot be enforced by CRD schema.
func (r *OrdsSrvsReconciler) validateSpec(ctx context.Context, req ctrl.Request, ordssrvs *dbapi.OrdsSrvs, rState *OrdsSrvsReconcileState) (bool, error) {
	// Access log forwarding needs HTTP access logging enabled so the sidecar has a file to tail.
	if ordssrvs.Spec.AccessLogForwarder.Enabled && !ordssrvs.Spec.GlobalSettings.EnableStandaloneAccessLog {
		invalidAccessLogForwarderMessage := "spec.accessLogForwarder.enabled requires spec.globalSettings.enable.standalone.access.log to be true"
		condition := metav1.Condition{
			Type:    typeAvailableORDS,
			Status:  metav1.ConditionFalse,
			Reason:  "InvalidSpec",
	...
	// Access log persistence can be shared by multiple Pods only when the PVC supports multi-writer access.
	if rState.AccessLogExternal &&
		(ordssrvs.Spec.WorkloadType == "DaemonSet" || ordssrvs.Spec.Replicas > 1) &&
		ordssrvs.Spec.AccessLogPersistence.AccessMode != string(corev1.ReadWriteMany) {
		var invalidAccessLogAccessModeMessage string
		if ordssrvs.Spec.WorkloadType == "DaemonSet" {
			invalidAccessLogAccessModeMessage = fmt.Sprintf("spec.accessLogPersistence.accessMode must be %q when spec.workloadType is DaemonSet", corev1.ReadWriteMany)
		} else {
			invalidAccessLogAccessModeMessage = fmt.Sprintf("spec.accessLogPersistence.accessMode must be %q when spec.replicas is greater than 1", corev1.ReadWriteMany)
		}
		condition := metav1.Condition{
			Type:    typeAvailableORDS,
			Status:  metav1.ConditionFalse,
	...
	// APEX installation persistence can be shared by multiple Pods only when the PVC supports multi-writer access.
	if rState.APEXInstallationExternal &&
		(ordssrvs.Spec.WorkloadType == "DaemonSet" || ordssrvs.Spec.Replicas > 1) &&
		ordssrvs.Spec.GlobalSettings.APEXInstallationPersistence.AccessMode != string(corev1.ReadWriteMany) {
		var invalidAPEXInstallationAccessModeMessage string
		if ordssrvs.Spec.WorkloadType == "DaemonSet" {
			invalidAPEXInstallationAccessModeMessage = fmt.Sprintf("spec.globalSettings.apex.installation.persistence.accessMode must be %q when spec.workloadType is DaemonSet", corev1.ReadWriteMany)
		} else {
			invalidAPEXInstallationAccessModeMessage = fmt.Sprintf("spec.globalSettings.apex.installation.persistence.accessMode must be %q when spec.replicas is greater than 1", corev1.ReadWriteMany)
		}
		condition := metav1.Condition{
			Type:    typeAvailableORDS,
			Status:  metav1.ConditionFalse,
...
func persistenceRequested(p dbapi.OrdsSrvsPersistence) bool {
	return p.Size != "" ||
		p.AccessMode != "" ||
		p.StorageClass != "" ||
		p.VolumeName != ""
