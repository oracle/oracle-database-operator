...
func (r *ShardingDatabaseReconciler) phasePrimaryShardOps(
	...
	if err := r.ensurePrimaryShardDataguardPrereqs(inst); err != nil {
		plog.Info("primary shard Data Guard prereqs not complete yet; requeue", "reason", "PrimaryShardDataguardPrereqsProgress", "error", err.Error(), "requeueAfter", 30*time.Second)
		return phaseResult{
			wait:         true,
			requeueAfter: 30 * time.Second,
			reason:       "PrimaryShardDataguardPrereqsProgress",
			message:      err.Error(),
		}
	}
...
func (r *ShardingDatabaseReconciler) publishEvents(instance *databasev4.ShardingDatabase, eventMsg string, state string) {
	...
	if state == string(databasev4.AvailableState) || state == string(databasev4.AddingShardState) || state == string(databasev4.ShardOnlineState) || state == string(databasev4.ProvisionState) || state == string(databasev4.DeletingState) || state == string(databasev4.Terminated) {
		r.Recorder.Eventf(instance, corev1.EventTypeNormal, "State Change", "%s", eventMsg)
	} else {
		r.Recorder.Eventf(instance, corev1.EventTypeWarning, "State Change", "%s", eventMsg)
...
func (r *ShardingDatabaseReconciler) validateSpex(instance *databasev4.ShardingDatabase) error {
	...
	if lastSuccSpec == nil {
		...
		if lastSuccSpec.DbImage != instance.Spec.DbImage {
			...
			r.Recorder.Eventf(instance, corev1.EventTypeWarning, eventErr, "%s", eventMsg)
...
func resolvePrimarySourcesList(sources []databasev4.StandbyPrimarySource) []resolvedStandbyPrimarySource {
	out := make([]resolvedStandbyPrimarySource, 0, len(sources))
	for i := range sources {
		source := sources[i]
...
func resolveStandbyPrimarySources(cfg *databasev4.StandbyConfig) []resolvedStandbyPrimarySource {
	...
	return resolvePrimarySourcesList(cfg.PrimarySources)
}
...
func (r *ShardingDatabaseReconciler) buildDgPairsFromStandbyConfig(instance *databasev4.ShardingDatabase) []databasev4.DgPairStatus {
	...
	if shardingType != "USER" {
		return r.buildDgPairsFromExternalPrimarySources(instance)
	}
...
func (r *ShardingDatabaseReconciler) buildDgPairsFromExternalPrimarySources(instance *databasev4.ShardingDatabase) []databasev4.DgPairStatus {
	shardingType := effectiveShardingTypeForConstraints(instance)
	...
	for i := range instance.Spec.ShardInfo {
		info := instance.Spec.ShardInfo[i]
		if len(info.ExternalPrimarySources) == 0 {
			continue
		}
		if !isStandbyShardInfoDeployAs(info.ShardGroupDetails, info.ShardSpaceDetails) {
			continue
		}
		identities := buildPrimaryIdentitiesFromSources(instance, info.ExternalPrimarySources, true)
		if len(identities) == 0 {
			continue
		}
		standbys := standbyShardMembersForInfo(instance, info)
		for idx := 0; idx < len(standbys) && idx < len(identities); idx++ {
			pairs = append(pairs, databasev4.DgPairStatus{
				PrimaryKey:           identities[idx].Key,
				PrimarySource:        identities[idx].Source,
				PrimaryConnectString: identities[idx].Connect,
				StandbyShardName:     standbys[idx].Name,
				StandbyShardNum:      int32(idx + 1),
				State:                "MAPPED",
				Message:              "mapped from externalPrimarySources",
			})
		}
	}
...
func buildPrimaryIdentities(instance *databasev4.ShardingDatabase, cfg *databasev4.StandbyConfig) []primaryIdentity {
	...
	return buildPrimaryIdentitiesFromSources(instance, cfg.PrimarySources, false)
}

func buildPrimaryIdentitiesFromSources(instance *databasev4.ShardingDatabase, sources []databasev4.StandbyPrimarySource, preserveOrder bool) []primaryIdentity {
	ids := []primaryIdentity{}
	...
	for _, resolved := range resolvePrimarySourcesList(sources) {
...
func isStandbyShardInfoDeployAs(group *databasev4.ShardGroupSpec, space *databasev4.ShardSpaceSpec) bool {
	deployAs := ""
	if group != nil {
		deployAs = group.DeployAs
	} else if space != nil {
		deployAs = space.DeployAs
	}
	v := strings.ToUpper(strings.TrimSpace(deployAs))
	return v == "STANDBY" || v == "ACTIVE_STANDBY"
}

func standbyShardMembersForInfo(instance *databasev4.ShardingDatabase, info databasev4.ShardingDetails) []databasev4.ShardSpec {
	prefix := strings.TrimSpace(info.ShardPreFixName)
	if prefix == "" {
		return nil
	}
	members := make([]databasev4.ShardSpec, 0)
	for i := range instance.Spec.Shard {
		...
		if strings.HasPrefix(shard.Name, prefix) {
			members = append(members, shard)
		}
	}
	sort.Slice(members, func(i, j int) bool {
		return strings.ToUpper(strings.TrimSpace(members[i].Name)) < strings.ToUpper(strings.TrimSpace(members[j].Name))
	})
	return members
}
...
func (r *ShardingDatabaseReconciler) ensurePrimaryRefForStandby(
	...
	changed := false
	if shardingType == "SYSTEM" || shardingType == "COMPOSITE" {
		externalChanged, err := r.ensureExternalPrimaryRefForStandby(instance)
		if err != nil {
			return false, err
		}
		changed = changed || externalChanged

		localChanged, err := r.ensureLocalPrimaryRefForStandby(instance)
		if err != nil {
			return false, err
		}
...
func buildLocalPrimaryDatabaseRef(instance *databasev4.ShardingDatabase, primary databasev4.ShardSpec) *databasev4.DatabaseRef {
	if instance == nil {
		return nil
	}
	...
	return &databasev4.DatabaseRef{
		Host:    fmt.Sprintf("%s-0.%s.%s.svc.cluster.local", name, name, instance.Namespace),
		Port:    1521,
		CdbName: upperName,
		PdbName: upperName + "PDB",
	}
}

func (r *ShardingDatabaseReconciler) ensureLocalPrimaryRefForStandby(instance *databasev4.ShardingDatabase) (bool, error) {
	if instance == nil {
		return false, nil
	}
	shardingType := effectiveShardingTypeForConstraints(instance)
	...
	for i := range instance.Spec.Shard {
		shard := &instance.Spec.Shard[i]
		if !isStandbyDeployAs(shard.DeployAs) {
			continue
		}
		if shard.PrimaryDatabaseRef != nil && strings.TrimSpace(shard.PrimaryDatabaseRef.Host) != "" {
			continue
		}

		if standbyConfigPrimaryCountController(shard.StandbyConfig) > 0 {
			continue
		}
		if info := matchShardInfoByLongestPrefix(instance, shard.Name); info != nil {
			if len(info.ExternalPrimarySources) > 0 || standbyConfigPrimaryCountController(info.StandbyConfig) > 0 {
				continue
			}
		}

		primary, err := r.findPrimaryForStandby(instance, *shard)
		if err != nil {
			return false, err
		}
		ref := buildLocalPrimaryDatabaseRef(instance, *primary)
		if ref == nil {
			continue
		}
		if !databaseRefEqual(shard.PrimaryDatabaseRef, ref) {
			shard.PrimaryDatabaseRef = ref
			changed = true
		}
	}
	return changed, nil
}
func (r *ShardingDatabaseReconciler) ensureExternalPrimaryRefForStandby(instance *databasev4.ShardingDatabase) (bool, error) {
	changed := false
	for i := range instance.Spec.ShardInfo {
		info := instance.Spec.ShardInfo[i]
		if len(info.ExternalPrimarySources) == 0 {
		...
		standbys := standbyShardMembersForInfo(instance, info)
		if len(standbys) != len(info.ExternalPrimarySources) {
			return false, fmt.Errorf("standby shardInfo prefix %s requires %d externalPrimarySources entries, found %d", strings.TrimSpace(info.ShardPreFixName), len(standbys), len(info.ExternalPrimarySources))
		}

		for idx := range standbys {
			ref, err := externalPrimaryDatabaseRef(info.ExternalPrimarySources[idx])
			if err != nil {
				return false, fmt.Errorf("standby shard %s external primary source %d: %w", standbys[idx].Name, idx, err)
			}
			for sj := range instance.Spec.Shard {
				if instance.Spec.Shard[sj].Name != standbys[idx].Name {
					continue
				}
				if !databaseRefEqual(instance.Spec.Shard[sj].PrimaryDatabaseRef, ref) {
					instance.Spec.Shard[sj].PrimaryDatabaseRef = ref
					changed = true
				}
				break
			}
...
func externalPrimaryDatabaseRef(source databasev4.StandbyPrimarySource) (*databasev4.DatabaseRef, error) {
	if source.Details == nil {
		return nil, fmt.Errorf("details is required")
	}
	host := strings.TrimSpace(source.Details.Host)
	cdb := strings.ToUpper(strings.TrimSpace(source.Details.CdbName))
	if host == "" || cdb == "" {
		return nil, fmt.Errorf("details.host and details.cdbName are required")
	}
	port := source.Details.Port
	if port <= 0 {
		port = 1521
	}
	return &databasev4.DatabaseRef{
		...
		Port:    port,
		CdbName: cdb,
		PdbName: strings.ToUpper(strings.TrimSpace(source.Details.PdbName)),
	}, nil
}

func databaseRefEqual(a, b *databasev4.DatabaseRef) bool {
	if a == nil || b == nil {
		return a == b
	}
	return strings.TrimSpace(a.Host) == strings.TrimSpace(b.Host) &&
		a.Port == b.Port &&
		strings.EqualFold(strings.TrimSpace(a.CdbName), strings.TrimSpace(b.CdbName)) &&
		strings.EqualFold(strings.TrimSpace(a.PdbName), strings.TrimSpace(b.PdbName))
}
...
func (r *ShardingDatabaseReconciler) ensureStandbyShardAddedInGsm(
	instance *databasev4.ShardingDatabase,
	gsmPod *corev1.Pod,
	...
	if gsmPod == nil || strings.TrimSpace(gsmPod.Name) == "" {
		return "", false, fmt.Errorf("GSM pod is empty")
	}
	if shardSfSet == nil || strings.TrimSpace(shardSfSet.Name) == "" {
		return "", false, fmt.Errorf("shard statefulset is empty for standby shard %s", shard.Name)
	}

	sparamsCheck := shardingv1.BuildShardParams(instance, shardSfSet, shard)
	if err := shardingv1.CheckShardInGsm(gsmPod.Name, sparamsCheck, instance, r.kubeConfig, r.Log); err == nil {
		return "", false, nil
	}

	sparamsAdd := shardingv1.BuildShardParamsForAdd(instance, shardSfSet, shard)
	if preErr := r.ensureShardAddPrerequisitesInGsm(instance, gsmPod.Name, shard); preErr != nil {
		return "", false, preErr
	}

	r.updateGsmShardStatus(instance, shard.Name, string(databasev4.AddingShardState))
	if err := shardingv1.AddShardInGsm(gsmPod.Name, sparamsAdd, instance, r.kubeConfig, r.Log); err != nil {
		return "", false, err
	}

	return sparamsAdd, true, nil
}
...
func (r *ShardingDatabaseReconciler) verifyShards(instance *databasev4.ShardingDatabase, gsmPod *corev1.Pod, shardSfSet *appsv1.StatefulSet, OraShardSpex databasev4.ShardSpec) error {
	...
	if shardingv1.EffectiveReplicationType(instance.Spec.ReplicationType) == "DG" {
		...
		if deployAs == "STANDBY" || deployAs == "ACTIVE_STANDBY" {
			if !r.dgBrokerDone(instance, OraShardSpex.Name) {
				r.logLegacy("INFO", "DG mode: skipping CheckOnlineShardInGsm for standby shard "+OraShardSpex.Name+" until DG broker setup completes", nil, instance, r.Log)
				return nil
			}
			r.logLegacy("INFO", "DG mode: checking standby shard "+OraShardSpex.Name+" online state in GSM after DG broker setup", nil, instance, r.Log)
		}
	}
...
func (r *ShardingDatabaseReconciler) addStandbyShards(_ context.Context, instance *databasev4.ShardingDatabase) error {
	...
	for i := int32(0); i < int32(len(instance.Spec.Shard)); i++ {
		...
		if !isDGReplication {
			sparamsAdd, added, addErr := r.ensureStandbyShardAddedInGsm(instance, gsmPod, shardSfSet, OraShardSpex)
			if addErr != nil {
		...
		if isDGReplication && r.dgBrokerDone(instance, OraShardSpex.Name) {
			...
			if added {
				r.logLegacy("INFO", "DG standby shard flow: added standby shard "+OraShardSpex.Name+" in GSM after DG broker setup", nil, instance, r.Log)
				deployParamsList = append(deployParamsList, sparamsAdd)
			}
		}
	}
...
func isManagedPrimaryShard(instance *databasev4.ShardingDatabase, shard databasev4.ShardSpec) bool {
	if instance == nil {
		return false
	}
	if shardingv1.CheckIsDeleteFlag(shard.IsDelete, instance, logr.Discard()) {
		return false
	}
	deployAs := strings.ToUpper(strings.TrimSpace(shard.DeployAs))
	return deployAs != "STANDBY" && deployAs != "ACTIVE_STANDBY"
}
...
func (r *ShardingDatabaseReconciler) ensurePrimaryShardDataguardPrereqs(instance *databasev4.ShardingDatabase) error {
	if !getShardingDataguardPrereqsEnabled(instance) {
		return nil
	}
	if instance == nil || len(instance.Spec.Shard) == 0 {
		return nil
	}

	for i := range instance.Spec.Shard {
		shard := instance.Spec.Shard[i]
		if !isManagedPrimaryShard(instance, shard) {
			continue
		}
		if _, _, err := r.validateShard(instance, shard, i); err != nil {
			return err
		}
		if err := r.ensureShardDataguardPrereqs(instance, shard); err != nil {
			return err
		}
	}
...
func (r *ShardingDatabaseReconciler) syncCredentialRotationNonBlocking(
	...
	if err := r.Get(ctx, types.NamespacedName{Name: secretName, Namespace: inst.Namespace}, secret); err != nil {
		...
		r.Recorder.Eventf(inst, corev1.EventTypeWarning, "CredentialSync", "%s", msg)
	...
	recordedFingerprint := strings.TrimSpace(anns[credentialSyncHashAnnotation])
	if recordedFingerprint == "" {
		if err := validateCredentialSecretMaterial(inst, secret); err != nil {
			msg := "credential baseline pre-check failed: " + err.Error()
			r.setCredentialSyncCondition(status, metav1.ConditionFalse, "BaselineFailed", msg)
			r.Recorder.Eventf(inst, corev1.EventTypeWarning, "CredentialSync", "%s", msg)
			return r.recordCredentialSyncFailure(ctx, nn, msg)
		}
		if err := r.recordCredentialSyncSuccess(ctx, nn, fingerprint); err != nil {
			msg := "credential baseline metadata update failed: " + err.Error()
			r.setCredentialSyncCondition(status, metav1.ConditionFalse, "MetadataUpdateFailed", msg)
			r.Recorder.Eventf(inst, corev1.EventTypeWarning, "CredentialSync", "%s", msg)
			return credentialRetryInitialBackoff
		}
		r.setCredentialSyncCondition(status, metav1.ConditionTrue, "Adopted", "credential baseline recorded without password reset")
		r.Recorder.Eventf(inst, corev1.EventTypeNormal, "CredentialSync", "Credential baseline recorded without password reset")
		return 0
	}

	if fingerprint == recordedFingerprint {
