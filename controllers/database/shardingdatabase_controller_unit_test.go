...
func TestShardingUnit_BuildDgPairsFromExternalPrimarySourcesSystem(t *testing.T) {
	inst := &databasev4.ShardingDatabase{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "shdb",
			Namespace: "shns",
			...
			ShardInfo: []databasev4.ShardingDetails{
				{
					ShardPreFixName: "std",
					ShardNum:        2,
					ShardGroupDetails: &databasev4.ShardGroupSpec{
						Name:     "sg-ash",
						Region:   "ashburn",
						DeployAs: "STANDBY",
					},
					ExternalPrimarySources: []databasev4.StandbyPrimarySource{
						{Details: &databasev4.PrimaryEndpointRef{Host: "pri1.example.com", Port: 1521, CdbName: "prim1"}},
						{Details: &databasev4.PrimaryEndpointRef{Host: "pri2.example.com", Port: 1522, CdbName: "prim2"}},
					},
				},
			},
			Shard: []databasev4.ShardSpec{
				{Name: "std1", ShardGroup: "sg-ash", DeployAs: "STANDBY"},
				{Name: "std2", ShardGroup: "sg-ash", DeployAs: "STANDBY"},
			},
		},
	}

	r := &ShardingDatabaseReconciler{}
	pairs := r.buildDgPairsFromStandbyConfig(inst)
	if len(pairs) != 2 {
		t.Fatalf("expected two DG pairs, got %#v", pairs)
	}
	if pairs[0].StandbyShardName != "std1" || pairs[0].PrimaryConnectString != "//pri1.example.com:1521/PRIM1_DGMGRL" {
		t.Fatalf("unexpected first pair %#v", pairs[0])
	}
	if pairs[1].StandbyShardName != "std2" || pairs[1].PrimaryConnectString != "//pri2.example.com:1522/PRIM2_DGMGRL" {
		t.Fatalf("unexpected second pair %#v", pairs[1])
	}
}

func TestShardingUnit_EnsurePrimaryRefForStandbyUsesExternalPrimarySources(t *testing.T) {
	inst := &databasev4.ShardingDatabase{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "shdb",
			Namespace: "shns",
			...
			ShardInfo: []databasev4.ShardingDetails{
				{
					...
					ExternalPrimarySources: []databasev4.StandbyPrimarySource{
						{Details: &databasev4.PrimaryEndpointRef{Host: "pri1.example.com", Port: 1521, CdbName: "prim1", PdbName: "pdb1"}},
						{Details: &databasev4.PrimaryEndpointRef{Host: "pri2.example.com", Port: 1522, CdbName: "prim2"}},
					},
				},
			},
	...
	r := &ShardingDatabaseReconciler{}
	changed, err := r.ensurePrimaryRefForStandby(context.Background(), inst)
	if err != nil {
		t.Fatalf("ensurePrimaryRefForStandby() error = %v", err)
	}
	if !changed {
		t.Fatalf("expected standby primary refs to be patched")
	}
	if inst.Spec.Shard[0].PrimaryDatabaseRef == nil || inst.Spec.Shard[0].PrimaryDatabaseRef.Host != "pri1.example.com" {
		t.Fatalf("expected first standby to get external primary ref, got %#v", inst.Spec.Shard[0].PrimaryDatabaseRef)
	}
	if inst.Spec.Shard[1].PrimaryDatabaseRef == nil || inst.Spec.Shard[1].PrimaryDatabaseRef.CdbName != "PRIM2" {
		t.Fatalf("expected second standby to get external primary ref, got %#v", inst.Spec.Shard[1].PrimaryDatabaseRef)
	}
}

func TestShardingUnit_EnsurePrimaryRefForStandbyUsesLocalSystemPrimaries(t *testing.T) {
	inst := &databasev4.ShardingDatabase{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "shdb",
			Namespace: "shns",
		...
		Spec: databasev4.ShardingDatabaseSpec{
			...
			ReplicationType: "DG",
			Shard: []databasev4.ShardSpec{
				{Name: "pshardp1", ShardGroup: "pshg1", DeployAs: "PRIMARY"},
				{Name: "pshardp2", ShardGroup: "pshg1", DeployAs: "PRIMARY"},
				{Name: "pshardp3", ShardGroup: "pshg1", DeployAs: "PRIMARY"},
				{Name: "sshardp1", ShardGroup: "sg-standby", DeployAs: "STANDBY"},
				{Name: "sshardp2", ShardGroup: "sg-standby", DeployAs: "STANDBY"},
				{Name: "sshardp3", ShardGroup: "sg-standby", DeployAs: "STANDBY"},
			},
		},
	}
	...
	if !changed {
		t.Fatalf("expected standby primary refs to be patched from local primaries")
	}

	wantHosts := []string{
		"pshardp1-0.pshardp1.shns.svc.cluster.local",
		"pshardp2-0.pshardp2.shns.svc.cluster.local",
		"pshardp3-0.pshardp3.shns.svc.cluster.local",
	}
	for i, idx := range []int{3, 4, 5} {
		ref := inst.Spec.Shard[idx].PrimaryDatabaseRef
		if ref == nil || ref.Host != wantHosts[i] {
			t.Fatalf("expected standby %s to get host %q, got %#v", inst.Spec.Shard[idx].Name, wantHosts[i], ref)
		}
	}
}

func TestShardingUnit_EnsurePrimaryRefForStandbyUsesLocalCompositePrimaries(t *testing.T) {
	inst := &databasev4.ShardingDatabase{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "shdb",
			Namespace: "shns",
		...
		Spec: databasev4.ShardingDatabaseSpec{
			...
			ReplicationType: "DG",
			Shard: []databasev4.ShardSpec{
				{Name: "cprmy1", ShardGroup: "primary-rw", ShardSpace: "space1", DeployAs: "PRIMARY"},
				{Name: "cprmy2", ShardGroup: "primary-rw", ShardSpace: "space1", DeployAs: "PRIMARY"},
				{Name: "cstdby1", ShardGroup: "standby-ro", ShardSpace: "space1", DeployAs: "STANDBY"},
				{Name: "cstdby2", ShardGroup: "standby-ro", ShardSpace: "space1", DeployAs: "STANDBY"},
			},
		},
	}
	...
	if !changed {
		t.Fatalf("expected composite standby primary refs to be patched from local primaries")
	}

	wantHosts := []string{
		"cprmy1-0.cprmy1.shns.svc.cluster.local",
		"cprmy2-0.cprmy2.shns.svc.cluster.local",
	}
...
func TestShardingUnit_CredentialSyncAdoptsMissingBaselineWithoutReset(t *testing.T) {
	scheme := runtime.NewScheme()
	if err := corev1.AddToScheme(scheme); err != nil {
		t.Fatalf("add corev1 scheme: %v", err)
	}
	...
	if delay := r.syncCredentialRotationNonBlocking(context.Background(), inst, status); delay != 0 {
		t.Fatalf("expected no retry delay for baseline adoption, got %s", delay)
	}
	...
	if got := strings.TrimSpace(updated.Annotations[credentialSyncHashAnnotation]); got == "" {
		t.Fatalf("expected credential sync hash to be recorded")
	}
	if _, ok := updated.Annotations[credentialSyncRetryCountAnnotation]; ok {
		t.Fatalf("expected retry annotations to be cleared, got %#v", updated.Annotations)
	}
	cond := meta.FindStatusCondition(status.CrdStatus, credentialSyncConditionType)
	if cond == nil || cond.Status != metav1.ConditionTrue || cond.Reason != "Adopted" {
		t.Fatalf("expected adopted credential sync condition, got %#v", cond)
	}
}
